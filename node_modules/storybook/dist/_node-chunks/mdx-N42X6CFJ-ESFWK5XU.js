...
import {
  ...
  start,
  toEstree
} from "./chunk-YQSIMXLR.js";
import "./chunk-YPHI3MHA.js";
...
function isIdentifierStart(code2, astral) {
  return code2 < 65 ? code2 === 36 : code2 < 91 ? !0 : code2 < 97 ? code2 === 95 : code2 < 123 ? !0 : code2 <= 65535 ? code2 >= 170 && nonASCIIidentifierStart.test(String.fromCharCode(code2)) : astral === !1 ? !1 : isInAstralSet(code2, astralIdentifierStartCodes);
}
function isIdentifierChar(code2, astral) {
  return code2 < 48 ? code2 === 36 : code2 < 58 ? !0 : code2 < 65 ? !1 : code2 < 91 ? !0 : code2 < 97 ? code2 === 95 : code2 < 123 ? !0 : code2 <= 65535 ? code2 >= 170 && nonASCIIidentifier.test(String.fromCharCode(code2)) : astral === !1 ? !1 : isInAstralSet(code2, astralIdentifierStartCodes) || isInAstralSet(code2, astralIdentifierCodes);
}
...
function codePointToString(code2) {
  return code2 <= 65535 ? String.fromCharCode(code2) : (code2 -= 65536, String.fromCharCode((code2 >> 10) + 55296, (code2 & 1023) + 56320));
}
...
var SourceLocation = function(p, start2, end) {
...
pp$8.parseBreakContinueStatement = function(node2, keyword) {
  ...
  return i === this.labels.length && this.raise(node2.start, "Unsyntactic " + keyword), this.finishNode(node2, isBreak ? "BreakStatement" : "ContinueStatement");
};
...
pp$8.parseClassElement = function(constructorAllowsSuper) {
  if (this.eat(types$1.semi))
    return null;
  var ecmaVersion = this.options.ecmaVersion, node2 = this.startNode(), keyName = "", isGenerator = !1, isAsync = !1, kind = "method", isStatic = !1;
  ...
  if (node2.static = isStatic, !keyName && ecmaVersion >= 8 && this.eatContextual("async") && ((this.isClassElementNameStart() || this.type === types$1.star) && !this.canInsertSemicolon() ? isAsync = !0 : keyName = "async"), !keyName && (ecmaVersion >= 9 || !isAsync) && this.eat(types$1.star) && (isGenerator = !0), !keyName && !isAsync && !isGenerator) {
    var lastValue = this.value;
    (this.eatContextual("get") || this.eatContextual("set")) && (this.isClassElementNameStart() ? kind = lastValue : keyName = lastValue);
  }
  if (keyName ? (node2.computed = !1, node2.key = this.startNodeAt(this.lastTokStart, this.lastTokStartLoc), node2.key.name = keyName, this.finishNode(node2.key, "Identifier")) : this.parseClassElementName(node2), ecmaVersion < 13 || this.type === types$1.parenL || kind !== "method" || isGenerator || isAsync) {
...
function checkKeyName(node2, name2) {
  var computed = node2.computed, key = node2.key;
...
pp$5.checkPropClash = function(prop, propHash, refDestructuringErrors) {
  if (!(this.options.ecmaVersion >= 9 && prop.type === "SpreadElement") && !(this.options.ecmaVersion >= 6 && (prop.computed || prop.method || prop.shorthand))) {
    ...
    var kind = prop.kind;
    if (this.options.ecmaVersion >= 6) {
      name2 === "__proto__" && kind === "init" && (propHash.proto && (refDestructuringErrors ? refDestructuringErrors.doubleProto < 0 && (refDestructuringErrors.doubleProto = key.start) : this.raiseRecoverable(key.start, "Redefinition of __proto__ property")), propHash.proto = !0);
      return;
    }
    name2 = "$" + name2;
...
pp$5.checkParams = function(node2, allowDuplicates) {
  for (var nameHash = /* @__PURE__ */ Object.create(null), i = 0, list3 = node2.params; i < list3.length; i += 1) {
    var param = list3[i];
...
for (i = 0, list3 = [9, 10, 11, 12, 13, 14]; i < list3.length; i += 1)
  ecmaVersion = list3[i], buildUnicodeData(ecmaVersion);
var ecmaVersion, i, list3, pp$1 = Parser.prototype, BranchID = function(parent, base) {
...
RegExpValidationState.prototype.eatChars = function(chs, forceU) {
  forceU === void 0 && (forceU = !1);
  for (var pos = this.pos, i = 0, list3 = chs; i < list3.length; i += 1) {
    var ch = list3[i], current2 = this.at(pos, forceU);
...
pp.readToken = function(code2) {
  return isIdentifierStart(code2, this.options.ecmaVersion >= 6) || code2 === 92 ? this.readWord() : this.getTokenFromCode(code2);
};
...
pp.readToken_mult_modulo_exp = function(code2) {
  var next = this.input.charCodeAt(this.pos + 1), size = 1, tokentype = code2 === 42 ? types$1.star : types$1.modulo;
  return this.options.ecmaVersion >= 7 && code2 === 42 && next === 42 && (++size, tokentype = types$1.starstar, next = this.input.charCodeAt(this.pos + 2)), next === 61 ? this.finishOp(types$1.assign, size + 1) : this.finishOp(tokentype, size);
};
pp.readToken_pipe_amp = function(code2) {
  ...
  return next === 61 ? this.finishOp(types$1.assign, 2) : this.finishOp(code2 === 124 ? types$1.bitwiseOR : types$1.bitwiseAND, 1);
};
...
pp.readToken_plus_min = function(code2) {
  var next = this.input.charCodeAt(this.pos + 1);
  return next === code2 ? next === 45 && !this.inModule && this.input.charCodeAt(this.pos + 2) === 62 && (this.lastTokEnd === 0 || lineBreak.test(this.input.slice(this.lastTokEnd, this.pos))) ? (this.skipLineComment(3), this.skipSpace(), this.nextToken()) : this.finishOp(types$1.incDec, 2) : next === 61 ? this.finishOp(types$1.assign, 2) : this.finishOp(types$1.plusMin, 1);
};
pp.readToken_lt_gt = function(code2) {
  var next = this.input.charCodeAt(this.pos + 1), size = 1;
  return next === code2 ? (size = code2 === 62 && this.input.charCodeAt(this.pos + 2) === 62 ? 3 : 2, this.input.charCodeAt(this.pos + size) === 61 ? this.finishOp(types$1.assign, size + 1) : this.finishOp(types$1.bitShift, size)) : next === 33 && code2 === 60 && !this.inModule && this.input.charCodeAt(this.pos + 2) === 45 && this.input.charCodeAt(this.pos + 3) === 45 ? (this.skipLineComment(4), this.skipSpace(), this.nextToken()) : (next === 61 && (size = 2), this.finishOp(types$1.relational, size));
};
...
pp.readToken_numberSign = function() {
  var ecmaVersion = this.options.ecmaVersion, code2 = 35;
  if (ecmaVersion >= 13 && (++this.pos, code2 = this.fullCharCodeAtPos(), isIdentifierStart(code2, !0) || code2 === 92))
...
pp.readInt = function(radix, len, maybeLegacyOctalNumericLiteral) {
  for (var allowSeparators = this.options.ecmaVersion >= 12 && len === void 0, isLegacyOctalNumericLiteral = maybeLegacyOctalNumericLiteral && this.input.charCodeAt(this.pos) === 48, start2 = this.pos, total = 0, lastCode = 0, i = 0, e = len ?? 1 / 0; i < e; ++i, ++this.pos) {
    var code2 = this.input.charCodeAt(this.pos), val = void 0;
    if (allowSeparators && code2 === 95) {
      isLegacyOctalNumericLiteral && this.raiseRecoverable(this.pos, "Numeric separator is not allowed in legacy octal numeric literals"), lastCode === 95 && this.raiseRecoverable(this.pos, "Numeric separator must be exactly one underscore"), i === 0 && this.raiseRecoverable(this.pos, "Numeric separator is not allowed at the first of digits"), lastCode = code2;
      continue;
    }
    if (code2 >= 97 ? val = code2 - 97 + 10 : code2 >= 65 ? val = code2 - 65 + 10 : code2 >= 48 && code2 <= 57 ? val = code2 - 48 : val = 1 / 0, val >= radix)
      break;
    lastCode = code2, total = total * radix + val;
  }
...
pp.readCodePoint = function() {
  ...
  if (ch === 123) {
    ...
    code2 = this.readHexChar(this.input.indexOf("}", this.pos) - this.pos), ++this.pos, code2 > 1114111 && this.invalidStringToken(codePos, "Code point out of bounds");
  } else
  ...
  "node_modules/micromark-extension-mdxjs/node_modules/acorn-jsx/index.js"(exports, module) {
    ...
    function getJsxTokens(acorn) {
      ...
      if (!acornJsx2) {
        ...
        }, acornJsx2 = { tokContexts, tokTypes }, acornJsxMap.set(acorn, acornJsx2);
      }
      return acornJsx2;
    }
    ...
    module.exports = function(options) {
      return options = options || {}, function(Parser22) {
        return plugin({
      ...
      get: function() {
        return getJsxTokens(__require("acorn")).tokTypes;
      },
      configurable: !0,
      enumerable: !0
    });
    function plugin(options, Parser22) {
      let acorn = Parser22.acorn || __require("acorn"), acornJsx2 = getJsxTokens(acorn), tt = acorn.tokTypes, tok = acornJsx2.tokTypes, tokContexts = acorn.tokContexts, tc_oTag = acornJsx2.tokContexts.tc_oTag, tc_cTag = acornJsx2.tokContexts.tc_cTag, tc_expr = acornJsx2.tokContexts.tc_expr, isNewLine2 = acorn.isNewLine, isIdentifierStart2 = acorn.isIdentifierStart, isIdentifierChar2 = acorn.isIdentifierChar;
      return class extends Parser22 {
...
`) : out = String.fromCharCode(ch), this.options.locations && (++this.curLine, this.lineStart = this.pos), out;
        ...
        readToken(code2) {
          ...
          return code2 === 60 && this.exprAllowed && this.input.charCodeAt(this.pos + 1) !== 33 ? (++this.pos, this.finishToken(tok.jsxTagStart)) : super.readToken(code2);
        }
  ...
  "node_modules/extend/index.js"(exports, module) {
    ...
    var hasOwn2 = Object.prototype.hasOwnProperty, toStr = Object.prototype.toString, defineProperty = Object.defineProperty, gOPD = Object.getOwnPropertyDescriptor, isArray2 = function(arr) {
      ...
      if (!obj || toStr.call(obj) !== "[object Object]")
        return !1;
      var hasOwnConstructor = hasOwn2.call(obj, "constructor"), hasIsPrototypeOf = obj.constructor && obj.constructor.prototype && hasOwn2.call(obj.constructor.prototype, "isPrototypeOf");
      if (obj.constructor && !hasOwnConstructor && !hasIsPrototypeOf)
        return !1;
      var key;
      for (key in obj)
        ;
      return typeof key > "u" || hasOwn2.call(obj, key);
    ...
    module.exports = function extend2() {
      var options, name2, src, copy, copyIsArray, clone, target = arguments[0], i = 1, length = arguments.length, deep = !1;
      for (typeof target == "boolean" && (deep = target, target = arguments[1] || {}, i = 2), (target == null || typeof target != "object" && typeof target != "function") && (target = {}); i < length; ++i)
        if (options = arguments[i], options != null)
          for (name2 in options)
            src = getProperty(target, name2), copy = getProperty(options, name2), target !== copy && (deep && copy && (isPlainObject2(copy) || (copyIsArray = isArray2(copy))) ? (copyIsArray ? (copyIsArray = !1, clone = src && isArray2(src) ? src : []) : clone = src && isPlainObject2(src) ? src : {}, setProperty(target, { name: name2, newValue: extend2(deep, clone, copy) })) : typeof copy < "u" && setProperty(target, { name: name2, newValue: copy }));
      return target;
    };
  }
});
function stringifyPosition(value) {
  return !value || typeof value != "object" ? "" : "position" in value || "type" in value ? position2(value.position) : "start" in value || "end" in value ? position2(value) : "line" in value || "column" in value ? point(value) : "";
}
...
var order = (
  ...
  constructor(value) {
    let options;
    value ? isUrl(value) ? options = { path: value } : typeof value == "string" || isUint8Array(value) ? options = { value } : options = value : options = {}, this.cwd = default3.cwd(), this.data = {}, this.history = [], this.messages = [], this.value, this.map, this.result, this.stored;
    let index2 = -1;
    for (; ++index2 < order.length; ) {
      let prop2 = order[index2];
      prop2 in options && options[prop2] !== void 0 && options[prop2] !== null && (this[prop2] = prop2 === "history" ? [...options[prop2]] : options[prop2]);
    }
    let prop;
    for (prop in options)
  ...
  set basename(basename) {
    assertNonEmpty(basename, "basename"), assertPart(basename, "basename"), this.path = default2.join(this.dirname || "", basename);
  }
  /**
   ...
   */
  set dirname(dirname) {
    assertPath(this.basename, "dirname"), this.path = default2.join(dirname || "", this.basename);
  }
  /**
   ...
   */
  set path(path) {
    isUrl(path) && (path = fileURLToPath(path)), assertNonEmpty(path, "path"), this.path !== path && this.history.push(path);
  }
  /**
   ...
   */
  set stem(stem) {
    assertNonEmpty(stem, "stem"), assertPart(stem, "stem"), this.path = default2.join(this.dirname || "", stem + (this.extname || ""));
  }
...
var markdownExtension = [
  ...
  "mkdown",
  "ron"
], markdown_extensions_default = markdownExtension, md = markdown_extensions_default.map(function(d) {
  return "." + d;
});
...
function parseEntities(value, options = {}) {
  let additional = typeof options.additional == "string" ? options.additional.charCodeAt(0) : options.additional, result = [], index2 = 0, lines = -1, queue = "", point3, indent2;
  options.position && ("start" in options.position || "indent" in options.position ? (indent2 = options.position.indent, point3 = options.position.start) : point3 = options.position);
  let line = (point3 ? point3.line : 0) || 1, column = (point3 ? point3.column : 0) || 1, previous2 = now(), character;
  for (index2--; ++index2 <= value.length; )
    if (character === 10 && (column = (indent2 ? indent2[lines] : 0) || 1), character = value.charCodeAt(index2), character === 38) {
      ...
      if (following === 35) {
        ...
        type = "named";
      let characterReferenceCharacters = "", characterReference2 = "", characters = "", test = type === "named" ? isAlphanumerical : type === "decimal" ? isDecimal : isHexadecimal;
      for (end--; ++end <= value.length; ) {
        let following2 = value.charCodeAt(end);
        if (!test(following2))
          break;
        characters += fromCharCode(following2), type === "named" && characterEntitiesLegacy.includes(characters) && (characterReferenceCharacters = characters, characterReference2 = decodeNamedCharacterReference(characters));
      }
      let terminated = value.charCodeAt(end) === 59;
      if (terminated) {
        end++;
        let namedReference = type === "named" ? decodeNamedCharacterReference(characters) : !1;
        namedReference && (characterReferenceCharacters = characters, characterReference2 = namedReference);
      }
      ...
      if (!(!terminated && options.nonTerminated === !1))
        if (!characters)
          ...
          if (prohibited(referenceCode))
            warning(7, diff), reference = fromCharCode(
              65533
              /* `�` */
            );
            ...
            let output = "";
            disallowed(referenceCode) && warning(6, diff), referenceCode > 65535 && (referenceCode -= 65536, output += fromCharCode(referenceCode >>> 10 | 55296), referenceCode = 56320 | referenceCode & 1023), reference = output + fromCharCode(referenceCode);
          }
        }
      if (reference) {
        flush(), previous2 = now(), index2 = end - 1, column += end - start2 + 1, result.push(reference);
        let next = now();
        next.offset++, options.reference && options.reference.call(
        ...
        characters = value.slice(start2 - 1, end), queue += characters, column += characters.length, index2 = end - 1;
    } else
  ...
  function warning(code2, offset2) {
    let position3;
    options.warning && (position3 = now(), position3.column += offset2, position3.offset += offset2, options.warning.call(
  ...
  function flush() {
    queue && (result.push(queue), options.text && options.text.call(options.textContext, queue, {
      start: previous2,
      end: now()
    }), queue = "");
  }
...
function mdxJsxFromMarkdown() {
  ...
  function enterMdxJsxTagAttribute(token) {
    let tag = this.data.mdxJsxTag;
    enterMdxJsxTagAnyAttribute.call(this, token), tag.attributes.push({ type: "mdxJsxAttribute", name: "", value: null });
  }
  function enterMdxJsxTagExpressionAttribute(token) {
    let tag = this.data.mdxJsxTag;
    enterMdxJsxTagAnyAttribute.call(this, token), tag.attributes.push({ type: "mdxJsxExpressionAttribute", value: "" }), this.buffer();
  }
  function exitMdxJsxTagExpressionAttribute(token) {
    ...
    tail.value = this.resume(), estree && (tail.data = { estree });
  }
  function exitMdxJsxTagAttributeNamePrimary(token) {
    let tag = this.data.mdxJsxTag;
    let node2 = tag.attributes[tag.attributes.length - 1];
    node2.type, node2.name = this.sliceSerialize(token);
  }
  ...
  function exitMdxJsxTagAttributeValueExpression(token) {
    ...
    estree && (node2.data = { estree }), tail.value = node2;
  }
  ...
  function onErrorRightIsTag(closing, open) {
    let tag = this.data.mdxJsxTag;
    let place = closing ? " before the end of `" + closing.type + "`" : "", position3 = closing ? { start: closing.start, end: closing.end } : void 0;
    throw new VFileMessage(
...
function mdxJsxToMarkdown(options) {
  let options_ = options || {}, quote = options_.quote || '"', quoteSmart = options_.quoteSmart || !1, tightSelfClosing = options_.tightSelfClosing || !1, printWidth = options_.printWidth || Number.POSITIVE_INFINITY, alternative = quote === '"' ? "'" : '"';
  if (quote !== '"' && quote !== "'")
    throw new Error(
      "Cannot serialize attribute values with `" + quote + "` for `options.quote`, expected `\"`, or `'`"
    );
  return mdxElement.peek = peekElement, {
    handlers: {
  ...
  function mdxElement(node2, _, state, info) {
    let flow3 = node2.type === "mdxJsxFlowElement", selfClosing = node2.name ? !node2.children || node2.children.length === 0 : !1, depth = inferDepth(state), currentIndent = createIndent(depth), trackerOneLine = state.createTracker(info), trackerMultiLine = state.createTracker(info), serializedAttributes = [], prefix = (flow3 ? currentIndent : "") + "<" + (node2.name || ""), exit2 = state.enter(node2.type);
    if (trackerOneLine.move(prefix), trackerMultiLine.move(prefix), node2.attributes && node2.attributes.length > 0) {
      if (!node2.name)
        throw new Error("Cannot serialize fragment w/ attributes");
      let index2 = -1;
      for (; ++index2 < node2.attributes.length; ) {
        let attribute = node2.attributes[index2], result;
        if (attribute.type === "mdxJsxExpressionAttribute")
          result = "{" + (attribute.value || "") + "}";
        else {
          if (!attribute.name)
            throw new Error("Cannot serialize attribute w/o name");
          let value2 = attribute.value, left = attribute.name, right = "";
          if (value2 != null)
            if (typeof value2 == "object")
              right = "{" + (value2.value || "") + "}";
            else {
              let appliedQuote = quoteSmart && ccount(value2, quote) > ccount(value2, alternative) ? alternative : quote;
    ...
    let attributesOnTheirOwnLine = !1, attributesOnOneLine = serializedAttributes.join(" ");
    // Block:
    ...
    (selfClosing ? tightSelfClosing ? 2 : 3 : 1) > printWidth) && (attributesOnTheirOwnLine = !0);
    let tracker = trackerOneLine, value = prefix;
    if (attributesOnTheirOwnLine) {
      tracker = trackerMultiLine;
      let index2 = -1;
      for (; ++index2 < serializedAttributes.length; )
...
` + currentIndent
    ...
    } else attributesOnOneLine && (value += tracker.move(" " + attributesOnOneLine));
    return selfClosing && (value += tracker.move(
      (tightSelfClosing || attributesOnTheirOwnLine ? "" : " ") + "/"
    )), value += tracker.move(">"), node2.children && node2.children.length > 0 && (node2.type === "mdxJsxTextElement" ? value += tracker.move(
    ...
    ) : (tracker.shift(2), value += tracker.move(`
`), value += tracker.move(containerFlow(node2, state, tracker.current())), value += tracker.move(`
`))), selfClosing || (value += tracker.move(
      (flow3 ? currentIndent : "") + "</" + (node2.name || "") + ">"
    )), exit2(), value;
  }
}
function containerFlow(parent, state, info) {
  let indexStack = state.indexStack, children = parent.children, tracker = state.createTracker(info), currentIndent = createIndent(inferDepth(state)), results = [], index2 = -1;
  for (indexStack.push(-1); ++index2 < children.length; ) {
    let child = children[index2];
    indexStack[indexStack.length - 1] = index2;
    let childInfo = { before: `
`, after: `
`, ...tracker.current() }, result = state.handle(child, parent, state, childInfo), serializedChild = child.type === "mdxJsxFlowElement" ? result : state.indentLines(result, function(line, _, blank) {
      return (blank ? "" : currentIndent) + line;
    });
    results.push(tracker.move(serializedChild)), child.type !== "list" && (state.bulletLastUsed = void 0), index2 < children.length - 1 && results.push(tracker.move(`
...
function inferDepth(state) {
  let depth = 0;
  for (let x of state.stack)
    x === "mdxJsxFlowElement" && depth++;
  return depth;
}
...
var import_acorn_jsx = __toESM(require_acorn_jsx(), 1), unicodePunctuationInternal = regexCheck(new RegExp("\\p{P}", "u")), asciiAlpha = regexCheck(/[A-Za-z]/), asciiAlphanumeric = regexCheck(/[\dA-Za-z]/), asciiAtext = regexCheck(/[#-'*+\--9=?A-Z^-~]/);
...
var own2 = {}.hasOwnProperty, CONTINUE = Symbol("continue"), EXIT = Symbol("exit"), SKIP = Symbol("skip");
function visit(tree, visitor) {
  ...
  function build(node2, key, index2, parents) {
    return nodelike(node2) && (visit3.displayName = "node (" + color(node2.type) + ")"), visit3;
    function visit3() {
      let result = enter ? toResult(enter(node2, key, index2, parents)) : [];
      if (result[0] === EXIT)
        return result;
      if (result[0] !== SKIP) {
        let cKey;
        for (cKey in node2)
          ...
          cKey !== "position") {
            ...
            if (Array.isArray(value)) {
              ...
              let subresult = build(value, cKey, void 0, grandparents)();
              if (subresult[0] === EXIT)
                return subresult;
            }
          }
...
function eventsToAcorn(events, options) {
  ...
  if (estree) {
    if (estree.comments = comments, visit(estree, function(esnode, field, index2, parents) {
      ...
      esnode.type === "ParenthesizedExpression" && context && prop && (typeof index2 == "number" && (context = context[prop], prop = index2), context[prop] = esnode.expression), fixPosition(esnode);
  ...
  function fixPosition(nodeOrToken) {
    let pointStart2 = parseOffsetToUnistPoint(nodeOrToken.start), pointEnd2 = parseOffsetToUnistPoint(nodeOrToken.end);
    nodeOrToken.start = pointStart2.offset, nodeOrToken.end = pointEnd2.offset, nodeOrToken.loc = {
      start: {
    ...
    if (event[0] === "enter") {
      ...
      if (type === "lineEnding" || tokenTypes.includes(type)) {
        ...
        for (; chunks.length > 0 && chunks[0] === -1; )
          chunks.shift();
        let value = serializeChunks(chunks);
...
function relativeToPoint(stops, relative) {
  let index2 = 0;
  for (; index2 < stops.length && stops[index2][0] <= relative; )
    index2 += 1;
  if (index2 === 0)
    return;
  let [stopRelative, stopAbsolute] = stops[index2 - 1], rest = relative - stopRelative;
  return {
...
function factoryMdxExpression(effects, ok3, type, markerType, chunkType, acorn, acornOptions, addResult, spread, allowEmpty, allowLazy) {
  ...
  function start2(code2) {
    return effects.enter(type), effects.enter(markerType), effects.consume(code2), effects.exit(markerType), pointStart2 = self2.now(), before;
  }
  function before(code2) {
    if (code2 === null) {
      if (lastCrash)
        throw lastCrash;
      let error = new VFileMessage(
      ...
      throw error.url = trouble + unexpectedEofHash, error;
    }
    if (markdownLineEnding(code2))
      return effects.enter("lineEnding"), effects.consume(code2), effects.exit("lineEnding"), eolAfter;
    if (code2 === 125 && size === 0) {
      let next = acorn ? mdxExpressionParse.call(
        self2,
        acorn,
        acornOptions,
        chunkType,
      ...
      if (next.type === "ok") {
        effects.enter(markerType), effects.consume(code2), effects.exit(markerType);
        let token = effects.exit(type);
        return addResult && next.estree && Object.assign(token, {
          estree: next.estree
        }), ok3;
      }
      return lastCrash = next.message, effects.enter(chunkType), effects.consume(code2), inside;
    }
    return effects.enter(chunkType), inside(code2);
  }
  function inside(code2) {
    return code2 === 125 && size === 0 || code2 === null || markdownLineEnding(code2) ? (effects.exit(chunkType), before(code2)) : (code2 === 123 && !acorn ? size += 1 : code2 === 125 && (size -= 1), effects.consume(code2), inside);
  }
...
function mdxExpressionParse(acorn, acornOptions, chunkType, eventStart, pointStart2, allowEmpty, spread) {
  ...
  if (spread && estree) {
    ...
    if (head.type !== "ExpressionStatement" || head.expression.type !== "ObjectExpression") {
      let place = positionFromEstree(head), error = new VFileMessage(
    ...
    if (head.expression.properties[1]) {
      let place = positionFromEstree(head.expression.properties[1]), error = new VFileMessage(
    ...
    if (head.expression.properties[0] && head.expression.properties[0].type !== "SpreadElement") {
      let place = positionFromEstree(head.expression.properties[0]), error = new VFileMessage(
...
function factorySpace(effects, ok3, type, max) {
  ...
  return start2;
  function start2(code2) {
    return markdownSpace(code2) ? (effects.enter(type), prefix(code2)) : ok3(code2);
  }
  function prefix(code2) {
    return markdownSpace(code2) && size++ < limit ? (effects.consume(code2), prefix) : (effects.exit(type), ok3(code2));
  }
}
function mdxExpression(options) {
  ...
  function tokenizeFlowExpression(effects, ok3, nok) {
    ...
    function end(code2) {
      ...
      return code2 === 60 && jsxTag ? effects.attempt(jsxTag, end, nok)(code2) : code2 === null || markdownLineEnding(code2) ? ok3(code2) : nok(code2);
    }
  }
  function tokenizeTextExpression(effects, ok3) {
    ...
    return start2;
    function start2(code2) {
      return factoryMdxExpression.call(self2, effects, ok3, "mdxTextExpression", "mdxTextExpressionMarker", "mdxTextExpressionChunk", acorn, acornOptions, addResult, spread, allowEmpty, !0)(code2);
    }
  }
}
...
function factoryTag(effects, ok3, nok, acorn, acornOptions, addResult, allowLazy, tagType, tagMarkerType, tagClosingMarkerType, tagSelfClosingMarker, tagNameType, tagNamePrimaryType, tagNameMemberMarkerType, tagNameMemberType, tagNamePrefixMarkerType, tagNameLocalType, tagExpressionAttributeType, tagExpressionAttributeMarkerType, tagExpressionAttributeValueType, tagAttributeType, tagAttributeNameType, tagAttributeNamePrimaryType, tagAttributeNamePrefixMarkerType, tagAttributeNameLocalType, tagAttributeInitializerMarkerType, tagAttributeValueLiteralType, tagAttributeValueLiteralMarkerType, tagAttributeValueLiteralValueType, tagAttributeValueExpressionType, tagAttributeValueExpressionMarkerType, tagAttributeValueExpressionValueType) {
  ...
  return start2;
  function start2(code2) {
    return effects.enter(tagType), effects.enter(tagMarkerType), effects.consume(code2), effects.exit(tagMarkerType), startAfter;
  }
  function startAfter(code2) {
    return markdownLineEndingOrSpace(code2) ? nok(code2) : (returnState = nameBefore, esWhitespaceStart(code2));
  }
  function nameBefore(code2) {
    if (code2 === 47)
      return effects.enter(tagClosingMarkerType), effects.consume(code2), effects.exit(tagClosingMarkerType), returnState = closingTagNameBefore, esWhitespaceStart;
    if (code2 === 62)
      return tagEnd(code2);
    if (code2 !== null && code2 >= 0 && start(code2))
      return effects.enter(tagNameType), effects.enter(tagNamePrimaryType), effects.consume(code2), primaryName;
  ...
  function primaryName(code2) {
    if (code2 !== null && code2 >= 0 && cont(code2, {
      ...
      return effects.consume(code2), primaryName;
    if (code2 === 46 || code2 === 47 || code2 === 58 || code2 === 62 || code2 === 123 || markdownLineEndingOrSpace(code2) || unicodeWhitespace(code2))
      return effects.exit(tagNamePrimaryType), returnState = primaryNameAfter, esWhitespaceStart(code2);
  ...
  function primaryNameAfter(code2) {
    if (code2 === 46)
      return effects.enter(tagNameMemberMarkerType), effects.consume(code2), effects.exit(tagNameMemberMarkerType), returnState = memberNameBefore, esWhitespaceStart;
    if (code2 === 58)
      return effects.enter(tagNamePrefixMarkerType), effects.consume(code2), effects.exit(tagNamePrefixMarkerType), returnState = localNameBefore, esWhitespaceStart;
    if (code2 === 47 || code2 === 62 || code2 === 123 || code2 !== null && code2 >= 0 && start(code2))
      return effects.exit(tagNameType), attributeBefore(code2);
  ...
  function memberNameBefore(code2) {
    if (code2 !== null && code2 >= 0 && start(code2))
      return effects.enter(tagNameMemberType), effects.consume(code2), memberName;
  ...
  function memberName(code2) {
    if (code2 !== null && code2 >= 0 && cont(code2, {
      ...
      return effects.consume(code2), memberName;
    if (code2 === 46 || code2 === 47 || code2 === 62 || code2 === 123 || markdownLineEndingOrSpace(code2) || unicodeWhitespace(code2))
      return effects.exit(tagNameMemberType), returnState = memberNameAfter, esWhitespaceStart(code2);
  ...
  function localNameBefore(code2) {
    if (code2 !== null && code2 >= 0 && start(code2))
      return effects.enter(tagNameLocalType), effects.consume(code2), localName;
  ...
  function localName(code2) {
    if (code2 !== null && code2 >= 0 && cont(code2, {
      ...
      return effects.consume(code2), localName;
    if (code2 === 47 || code2 === 62 || code2 === 123 || markdownLineEndingOrSpace(code2) || unicodeWhitespace(code2))
      return effects.exit(tagNameLocalType), returnState = localNameAfter, esWhitespaceStart(code2);
  ...
  function attributeBefore(code2) {
    if (code2 === 47)
      return effects.enter(tagSelfClosingMarker), effects.consume(code2), effects.exit(tagSelfClosingMarker), returnState = selfClosing, esWhitespaceStart;
    if (code2 === 62)
      return tagEnd(code2);
    if (code2 === 123)
      return factoryMdxExpression.call(self2, effects, attributeExpressionAfter, tagExpressionAttributeType, tagExpressionAttributeMarkerType, tagExpressionAttributeValueType, acorn, acornOptions, addResult, !0, !1, allowLazy)(code2);
    if (code2 !== null && code2 >= 0 && start(code2))
      return effects.enter(tagAttributeType), effects.enter(tagAttributeNameType), effects.enter(tagAttributeNamePrimaryType), effects.consume(code2), attributePrimaryName;
  ...
  function attributeExpressionAfter(code2) {
    return returnState = attributeBefore, esWhitespaceStart(code2);
  }
  function attributePrimaryName(code2) {
    if (code2 !== null && code2 >= 0 && cont(code2, {
      jsx: !0
    }))
      return effects.consume(code2), attributePrimaryName;
    if (code2 === 47 || code2 === 58 || code2 === 61 || code2 === 62 || code2 === 123 || markdownLineEndingOrSpace(code2) || unicodeWhitespace(code2))
      return effects.exit(tagAttributeNamePrimaryType), returnState = attributePrimaryNameAfter, esWhitespaceStart(code2);
  ...
  function attributePrimaryNameAfter(code2) {
    if (code2 === 58)
      return effects.enter(tagAttributeNamePrefixMarkerType), effects.consume(code2), effects.exit(tagAttributeNamePrefixMarkerType), returnState = attributeLocalNameBefore, esWhitespaceStart;
    if (code2 === 61)
      return effects.exit(tagAttributeNameType), effects.enter(tagAttributeInitializerMarkerType), effects.consume(code2), effects.exit(tagAttributeInitializerMarkerType), returnState = attributeValueBefore, esWhitespaceStart;
    if (code2 === 47 || code2 === 62 || code2 === 123 || markdownLineEndingOrSpace(code2) || unicodeWhitespace(code2) || code2 !== null && code2 >= 0 && start(code2))
      return effects.exit(tagAttributeNameType), effects.exit(tagAttributeType), returnState = attributeBefore, esWhitespaceStart(code2);
  ...
  function attributeLocalNameBefore(code2) {
    if (code2 !== null && code2 >= 0 && start(code2))
      return effects.enter(tagAttributeNameLocalType), effects.consume(code2), attributeLocalName;
  ...
  function attributeLocalName(code2) {
    if (code2 !== null && code2 >= 0 && cont(code2, {
      ...
      return effects.consume(code2), attributeLocalName;
    if (code2 === 47 || code2 === 61 || code2 === 62 || code2 === 123 || markdownLineEndingOrSpace(code2) || unicodeWhitespace(code2))
      return effects.exit(tagAttributeNameLocalType), effects.exit(tagAttributeNameType), returnState = attributeLocalNameAfter, esWhitespaceStart(code2);
  ...
  function attributeLocalNameAfter(code2) {
    if (code2 === 61)
      return effects.enter(tagAttributeInitializerMarkerType), effects.consume(code2), effects.exit(tagAttributeInitializerMarkerType), returnState = attributeValueBefore, esWhitespaceStart;
    if (code2 === 47 || code2 === 62 || code2 === 123 || code2 !== null && code2 >= 0 && start(code2))
      return effects.exit(tagAttributeType), attributeBefore(code2);
  ...
  function attributeValueBefore(code2) {
    if (code2 === 34 || code2 === 39)
      return effects.enter(tagAttributeValueLiteralType), effects.enter(tagAttributeValueLiteralMarkerType), effects.consume(code2), effects.exit(tagAttributeValueLiteralMarkerType), marker = code2, attributeValueQuotedStart;
    if (code2 === 123)
      return factoryMdxExpression.call(self2, effects, attributeValueExpressionAfter, tagAttributeValueExpressionType, tagAttributeValueExpressionMarkerType, tagAttributeValueExpressionValueType, acorn, acornOptions, addResult, !1, !1, allowLazy)(code2);
  ...
  function tagEnd(code2) {
    return effects.enter(tagMarkerType), effects.consume(code2), effects.exit(tagMarkerType), effects.exit(tagType), ok3;
  }
  function esWhitespaceStart(code2) {
    return markdownLineEnding(code2) ? (effects.enter("lineEnding"), effects.consume(code2), effects.exit("lineEnding"), esWhitespaceEolAfter) : markdownSpace(code2) || unicodeWhitespace(code2) ? (effects.enter("esWhitespace"), esWhitespaceInside(code2)) : returnState(code2);
  }
  function esWhitespaceInside(code2) {
    return markdownLineEnding(code2) ? (effects.exit("esWhitespace"), esWhitespaceStart(code2)) : markdownSpace(code2) || unicodeWhitespace(code2) ? (effects.consume(code2), esWhitespaceInside) : (effects.exit("esWhitespace"), returnState(code2));
  }
  ...
  function crash(code2, at2, expect) {
    let error = new VFileMessage("Unexpected " + (code2 === null ? "end of file" : "character `" + (code2 === 96 ? "` ` `" : String.fromCodePoint(code2)) + "` (" + serializeCharCode(code2) + ")") + " " + at2 + ", expected " + expect, self2.now(), "micromark-extension-mdx-jsx:unexpected-" + (code2 === null ? "eof" : "character"));
    throw error.url = trouble2 + (code2 === null ? "#unexpected-end-of-file-at-expected-expect" : "#unexpected-character-at-expected-expect"), error;
  }
}
...
function jsxText(acorn, options) {
  ...
  function tokenizeJsxText(effects, ok3, nok) {
    return factoryTag.call(this, effects, ok3, nok, acorn, options.acornOptions, options.addResult, !0, "mdxJsxTextTag", "mdxJsxTextTagMarker", "mdxJsxTextTagClosingMarker", "mdxJsxTextTagSelfClosingMarker", "mdxJsxTextTagName", "mdxJsxTextTagNamePrimary", "mdxJsxTextTagNameMemberMarker", "mdxJsxTextTagNameMember", "mdxJsxTextTagNamePrefixMarker", "mdxJsxTextTagNameLocal", "mdxJsxTextTagExpressionAttribute", "mdxJsxTextTagExpressionAttributeMarker", "mdxJsxTextTagExpressionAttributeValue", "mdxJsxTextTagAttribute", "mdxJsxTextTagAttributeName", "mdxJsxTextTagAttributeNamePrimary", "mdxJsxTextTagAttributeNamePrefixMarker", "mdxJsxTextTagAttributeNameLocal", "mdxJsxTextTagAttributeInitializerMarker", "mdxJsxTextTagAttributeValueLiteral", "mdxJsxTextTagAttributeValueLiteralMarker", "mdxJsxTextTagAttributeValueLiteralValue", "mdxJsxTextTagAttributeValueExpression", "mdxJsxTextTagAttributeValueExpressionMarker", "mdxJsxTextTagAttributeValueExpressionValue");
  }
}
function jsxFlow(acorn, options) {
  ...
  function tokenizeJsxFlow(effects, ok3, nok) {
    ...
    function end(code2) {
      let leftBraceValue = self2.parser.constructs.flow[123], expression = (Array.isArray(leftBraceValue) ? leftBraceValue : leftBraceValue ? [leftBraceValue] : []).find((d) => d.name === "mdxFlowExpression");
      return code2 === 60 ? (
...
function resolveAllAttention(events, context) {
  let index2 = -1, open, group, text4, openingSequence, closingSequence, use, nextEvents, offset2;
  for (; ++index2 < events.length; )
...
function tokenizeAttention(effects, ok3) {
  ...
  function inside(code2) {
    ...
    return token._open = !!(marker === 42 ? open : open && (before || !close)), token._close = !!(marker === 42 ? close : close && (after || !open)), ok3(code2);
  }
}
...
function tokenizeAutolink(effects, ok3, nok) {
  ...
  function schemeInsideOrEmailAtext(code2) {
    return code2 === 58 ? (effects.consume(code2), size = 0, urlInside) : (code2 === 43 || code2 === 45 || code2 === 46 || asciiAlphanumeric(code2)) && size++ < 32 ? (effects.consume(code2), schemeInsideOrEmailAtext) : (size = 0, emailAtext(code2));
  }
  function urlInside(code2) {
    return code2 === 62 ? (effects.exit("autolinkProtocol"), effects.enter("autolinkMarker"), effects.consume(code2), effects.exit("autolinkMarker"), effects.exit("autolink"), ok3) : code2 === null || code2 === 32 || code2 === 60 || asciiControl(code2) ? nok(code2) : (effects.consume(code2), urlInside);
  }
  ...
  function emailLabel(code2) {
    return code2 === 46 ? (effects.consume(code2), size = 0, emailAtSignOrDot) : code2 === 62 ? (effects.exit("autolinkProtocol").type = "autolinkEmail", effects.enter("autolinkMarker"), effects.consume(code2), effects.exit("autolinkMarker"), effects.exit("autolink"), ok3) : emailValue(code2);
  }
  function emailValue(code2) {
    if ((code2 === 45 || asciiAlphanumeric(code2)) && size++ < 63) {
      let next = code2 === 45 ? emailValue : emailLabel;
...
function tokenizeBlockQuoteStart(effects, ok3, nok) {
  ...
  function after(code2) {
    return markdownSpace(code2) ? (effects.enter("blockQuotePrefixWhitespace"), effects.consume(code2), effects.exit("blockQuotePrefixWhitespace"), effects.exit("blockQuotePrefix"), ok3) : (effects.exit("blockQuotePrefix"), ok3(code2));
  }
}
...
function tokenizeCharacterEscape(effects, ok3, nok) {
  ...
  function inside(code2) {
    return asciiPunctuation(code2) ? (effects.enter("characterEscapeValue"), effects.consume(code2), effects.exit("characterEscapeValue"), effects.exit("characterEscape"), ok3) : nok(code2);
  }
}
...
function tokenizeCharacterReference(effects, ok3, nok) {
  ...
  function value(code2) {
    if (code2 === 59 && size) {
      ...
      return test === asciiAlphanumeric && !decodeNamedCharacterReference(self2.sliceSerialize(token)) ? nok(code2) : (effects.enter("characterReferenceMarker"), effects.consume(code2), effects.exit("characterReferenceMarker"), effects.exit("characterReference"), ok3);
    }
    return test(code2) && size++ < max ? (effects.consume(code2), value) : nok(code2);
  }
}
...
function tokenizeCodeFenced(effects, ok3, nok) {
  ...
  function infoBefore(code2) {
    return code2 === null || markdownLineEnding(code2) ? (effects.exit("codeFencedFence"), self2.interrupt ? ok3(code2) : effects.check(nonLazyContinuation, atNonLazyBreak, after)(code2)) : (effects.enter("codeFencedFenceInfo"), effects.enter("chunkString", {
      contentType: "string"
    }), info(code2));
  }
  function info(code2) {
  ...
  function after(code2) {
    return effects.exit("codeFenced"), ok3(code2);
  }
  function tokenizeCloseStart(effects2, ok4, nok2) {
    ...
    function sequenceClose(code2) {
      return code2 === marker ? (size++, effects2.consume(code2), sequenceClose) : size >= sizeOpen ? (effects2.exit("codeFencedFenceSequence"), markdownSpace(code2) ? factorySpace(effects2, sequenceCloseAfter, "whitespace")(code2) : sequenceCloseAfter(code2)) : nok2(code2);
    }
    function sequenceCloseAfter(code2) {
      return code2 === null || markdownLineEnding(code2) ? (effects2.exit("codeFencedFence"), ok4(code2)) : nok2(code2);
    }
  }
}
...
function tokenizeCodeIndented(effects, ok3, nok) {
  ...
  function after(code2) {
    return effects.exit("codeIndented"), ok3(code2);
  }
}
function tokenizeFurtherStart(effects, ok3, nok) {
  ...
  function afterPrefix(code2) {
    let tail = self2.events[self2.events.length - 1];
    return tail && tail[1].type === "linePrefix" && tail[2].sliceSerialize(tail[1], !0).length >= 4 ? ok3(code2) : markdownLineEnding(code2) ? furtherStart2(code2) : nok(code2);
  }
}
var codeText = {
  name: "codeText",
...
function tokenizeCodeText(effects, ok3, nok) {
  ...
  function data2(code2) {
    return code2 === null || code2 === 32 || code2 === 96 || markdownLineEnding(code2) ? (effects.exit("codeTextData"), between(code2)) : (effects.consume(code2), data2);
  }
  function sequenceClose(code2) {
    return code2 === 96 ? (effects.consume(code2), size++, sequenceClose) : size === sizeOpen ? (effects.exit("codeTextSequence"), effects.exit("codeText"), ok3(code2)) : (token.type = "codeTextData", data2(code2));
  }
}
...
function tokenizeContent(effects, ok3) {
  ...
  function contentEnd(code2) {
    return effects.exit("chunkContent"), effects.exit("content"), ok3(code2);
  }
...
function tokenizeContinuation(effects, ok3, nok) {
  ...
  function prefixed(code2) {
    if (code2 === null || markdownLineEnding(code2))
      return nok(code2);
    let tail = self2.events[self2.events.length - 1];
    return !self2.parser.constructs.disable.null.includes("codeIndented") && tail && tail[1].type === "linePrefix" && tail[2].sliceSerialize(tail[1], !0).length >= 4 ? ok3(code2) : effects.interrupt(self2.parser.constructs.flow, nok, ok3)(code2);
  }
}
function factoryDestination(effects, ok3, nok, type, literalType, literalMarkerType, rawType, stringType, max) {
  ...
  function enclosedBefore(code2) {
    return code2 === 62 ? (effects.enter(literalMarkerType), effects.consume(code2), effects.exit(literalMarkerType), effects.exit(literalType), effects.exit(type), ok3) : (effects.enter(stringType), effects.enter("chunkString", {
      contentType: "string"
    }), enclosed(code2));
  }
...
function factoryLabel(effects, ok3, nok, type, markerType, stringType) {
  ...
  function atBreak(code2) {
    return size > 999 || code2 === null || code2 === 91 || code2 === 93 && !seen || // To do: remove in the future once we’ve switched from
    // `micromark-extension-footnote` to `micromark-extension-gfm-footnote`,
    // which doesn’t need this.
    // Hidden footnotes hook.
    /* c8 ignore next 3 */
    code2 === 94 && !size && "_hiddenFootnoteSupport" in self2.parser.constructs ? nok(code2) : code2 === 93 ? (effects.exit(stringType), effects.enter(markerType), effects.consume(code2), effects.exit(markerType), effects.exit(type), ok3) : markdownLineEnding(code2) ? (effects.enter("lineEnding"), effects.consume(code2), effects.exit("lineEnding"), atBreak) : (effects.enter("chunkString", {
      contentType: "string"
    }), labelInside(code2));
  }
  function labelInside(code2) {
    return code2 === null || code2 === 91 || code2 === 93 || markdownLineEnding(code2) || size++ > 999 ? (effects.exit("chunkString"), atBreak(code2)) : (effects.consume(code2), seen || (seen = !markdownSpace(code2)), code2 === 92 ? labelEscape : labelInside);
  }
  function labelEscape(code2) {
    return code2 === 91 || code2 === 92 || code2 === 93 ? (effects.consume(code2), size++, labelInside) : labelInside(code2);
  }
}
function factoryTitle(effects, ok3, nok, type, markerType, stringType) {
  ...
  function begin(code2) {
    return code2 === marker ? (effects.enter(markerType), effects.consume(code2), effects.exit(markerType), effects.exit(type), ok3) : (effects.enter(stringType), atBreak(code2));
  }
  function atBreak(code2) {
...
function tokenizeDefinition(effects, ok3, nok) {
  ...
  function afterWhitespace(code2) {
    return code2 === null || markdownLineEnding(code2) ? (effects.exit("definition"), self2.parser.defined.push(identifier), ok3(code2)) : nok(code2);
  }
}
...
function tokenizeHardBreakEscape(effects, ok3, nok) {
  ...
  function after(code2) {
    return markdownLineEnding(code2) ? (effects.exit("hardBreakEscape"), ok3(code2)) : nok(code2);
  }
}
var headingAtx = {
  name: "headingAtx",
...
function tokenizeHeadingAtx(effects, ok3, nok) {
  ...
  function sequenceOpen(code2) {
    return code2 === 35 && size++ < 6 ? (effects.consume(code2), sequenceOpen) : code2 === null || markdownLineEndingOrSpace(code2) ? (effects.exit("atxHeadingSequence"), atBreak(code2)) : nok(code2);
  }
  function atBreak(code2) {
    return code2 === 35 ? (effects.enter("atxHeadingSequence"), sequenceFurther(code2)) : code2 === null || markdownLineEnding(code2) ? (effects.exit("atxHeading"), ok3(code2)) : markdownSpace(code2) ? factorySpace(effects, atBreak, "whitespace")(code2) : (effects.enter("atxHeadingText"), data2(code2));
  }
  function sequenceFurther(code2) {
    return code2 === 35 ? (effects.consume(code2), sequenceFurther) : (effects.exit("atxHeadingSequence"), atBreak(code2));
  }
  function data2(code2) {
    return code2 === null || code2 === 35 || markdownLineEndingOrSpace(code2) ? (effects.exit("atxHeadingText"), atBreak(code2)) : (effects.consume(code2), data2);
  }
}
var htmlBlockNames = [
  "address",
...
function tokenizeHtmlFlow(effects, ok3, nok) {
  ...
  function open(code2) {
    return code2 === 33 ? (effects.consume(code2), declarationOpen) : code2 === 47 ? (effects.consume(code2), closingTag = !0, tagCloseStart) : code2 === 63 ? (effects.consume(code2), marker = 3, self2.interrupt ? ok3 : continuationDeclarationInside) : asciiAlpha(code2) ? (effects.consume(code2), buffer = String.fromCharCode(code2), tagName) : nok(code2);
  }
  function declarationOpen(code2) {
    return code2 === 45 ? (effects.consume(code2), marker = 2, commentOpenInside) : code2 === 91 ? (effects.consume(code2), marker = 5, index2 = 0, cdataOpenInside) : asciiAlpha(code2) ? (effects.consume(code2), marker = 4, self2.interrupt ? ok3 : continuationDeclarationInside) : nok(code2);
  }
  function commentOpenInside(code2) {
    return code2 === 45 ? (effects.consume(code2), self2.interrupt ? ok3 : continuationDeclarationInside) : nok(code2);
  }
  function cdataOpenInside(code2) {
    let value = "CDATA[";
    return code2 === value.charCodeAt(index2++) ? (effects.consume(code2), index2 === value.length ? self2.interrupt ? ok3 : continuation : cdataOpenInside) : nok(code2);
  }
  ...
  function tagName(code2) {
    if (code2 === null || code2 === 47 || code2 === 62 || markdownLineEndingOrSpace(code2)) {
      let slash = code2 === 47, name2 = buffer.toLowerCase();
      return !slash && !closingTag && htmlRawNames.includes(name2) ? (marker = 1, self2.interrupt ? ok3(code2) : continuation(code2)) : htmlBlockNames.includes(buffer.toLowerCase()) ? (marker = 6, slash ? (effects.consume(code2), basicSelfClosing) : self2.interrupt ? ok3(code2) : continuation(code2)) : (marker = 7, self2.interrupt && !self2.parser.lazy[self2.now().line] ? nok(code2) : closingTag ? completeClosingTagAfter(code2) : completeAttributeNameBefore(code2));
    }
    return code2 === 45 || asciiAlphanumeric(code2) ? (effects.consume(code2), buffer += String.fromCharCode(code2), tagName) : nok(code2);
  }
  function basicSelfClosing(code2) {
    return code2 === 62 ? (effects.consume(code2), self2.interrupt ? ok3 : continuation) : nok(code2);
  }
  ...
  function continuationAfter(code2) {
    return effects.exit("htmlFlow"), ok3(code2);
  }
}
...
function tokenizeBlankLineBefore(effects, ok3, nok) {
  return start2;
  function start2(code2) {
    return effects.enter("lineEnding"), effects.consume(code2), effects.exit("lineEnding"), effects.attempt(blankLine, ok3, nok);
  }
}
var htmlText = {
  name: "htmlText",
...
function tokenizeHtmlText(effects, ok3, nok) {
  ...
  function end(code2) {
    return code2 === 62 ? (effects.consume(code2), effects.exit("htmlTextData"), effects.exit("htmlText"), ok3) : nok(code2);
  }
...
function resolveToLabelEnd(events, context) {
  ...
  return media = [
    ["enter", group, context],
    ["enter", label, context]
  ], media = push(media, events.slice(open + 1, open + offset2 + 3)), media = push(media, [["enter", text4, context]]), media = push(
    media,
    resolveAll(
      context.parser.constructs.insideSpan.null,
...
function tokenizeResource(effects, ok3, nok) {
  ...
  function resourceEnd(code2) {
    return code2 === 41 ? (effects.enter("resourceMarker"), effects.consume(code2), effects.exit("resourceMarker"), effects.exit("resource"), ok3) : nok(code2);
  }
}
...
function tokenizeReferenceCollapsed(effects, ok3, nok) {
  ...
  function referenceCollapsedOpen(code2) {
    return code2 === 93 ? (effects.enter("referenceMarker"), effects.consume(code2), effects.exit("referenceMarker"), effects.exit("reference"), ok3) : nok(code2);
  }
}
var labelStartImage = {
...
function tokenizeLineEnding(effects, ok3) {
  return start2;
  function start2(code2) {
    return effects.enter("lineEnding"), effects.consume(code2), effects.exit("lineEnding"), factorySpace(effects, ok3, "linePrefix");
  }
}
var thematicBreak = {
  name: "thematicBreak",
...
function tokenizeThematicBreak(effects, ok3, nok) {
  ...
  function atBreak(code2) {
    return code2 === marker ? (effects.enter("thematicBreakSequence"), sequence(code2)) : size >= 3 && (code2 === null || markdownLineEnding(code2)) ? (effects.exit("thematicBreak"), ok3(code2)) : nok(code2);
  }
  function sequence(code2) {
    return code2 === marker ? (effects.consume(code2), size++, sequence) : (effects.exit("thematicBreakSequence"), markdownSpace(code2) ? factorySpace(effects, atBreak, "whitespace")(code2) : atBreak(code2));
  }
}
var list = {
  name: "list",
...
function tokenizeListStart(effects, ok3, nok) {
  ...
  return start2;
  function start2(code2) {
    let kind = self2.containerState.type || (code2 === 42 || code2 === 43 || code2 === 45 ? "listUnordered" : "listOrdered");
  ...
  function inside(code2) {
    return asciiDigit(code2) && ++size < 10 ? (effects.consume(code2), inside) : (!self2.interrupt || size < 2) && (self2.containerState.marker ? code2 === self2.containerState.marker : code2 === 41 || code2 === 46) ? (effects.exit("listItemValue"), atMarker(code2)) : nok(code2);
  }
  ...
  function endOfPrefix(code2) {
    return self2.containerState.size = initialSize + self2.sliceSerialize(effects.exit("listItemPrefix"), !0).length, ok3(code2);
  }
}
function tokenizeListContinuation(effects, ok3, nok) {
  ...
  function notBlank(code2) {
    return self2.containerState.furtherBlankLines || !markdownSpace(code2) ? (self2.containerState.furtherBlankLines = void 0, self2.containerState.initialBlankLine = void 0, notInCurrentItem(code2)) : (self2.containerState.furtherBlankLines = void 0, self2.containerState.initialBlankLine = void 0, effects.attempt(indentConstruct, ok3, notInCurrentItem)(code2));
  }
...
function tokenizeIndent(effects, ok3, nok) {
  ...
  function afterPrefix(code2) {
    let tail = self2.events[self2.events.length - 1];
    return tail && tail[1].type === "listItemIndent" && tail[2].sliceSerialize(tail[1], !0).length === self2.containerState.size ? ok3(code2) : nok(code2);
  }
}
...
function tokenizeSetextUnderline(effects, ok3, nok) {
  ...
  function after(code2) {
    return code2 === null || markdownLineEnding(code2) ? (effects.exit("setextHeadingLine"), ok3(code2)) : nok(code2);
  }
}
...
function mdxjsEsm(options) {
  ...
  function tokenizeExportImport(effects, ok3, nok) {
    ...
    function start2(code2) {
      return self2.now().column > 1 ? nok(code2) : (effects.enter("mdxjsEsm"), effects.enter("mdxjsEsmData"), effects.consume(code2), buffer += String.fromCharCode(code2), word);
    }
    function word(code2) {
      return asciiAlpha(code2) ? (effects.consume(code2), buffer += String.fromCharCode(code2), word) : (buffer === "import" || buffer === "export") && code2 === 32 ? (effects.consume(code2), inside) : nok(code2);
    }
    function inside(code2) {
      return code2 === null || markdownLineEnding(code2) ? (effects.exit("mdxjsEsmData"), lineStart(code2)) : (effects.consume(code2), inside);
    }
    function lineStart(code2) {
      return code2 === null ? atEnd(code2) : markdownLineEnding(code2) ? effects.check(blankLineBefore2, atEnd, continuationStart)(code2) : (effects.enter("mdxjsEsmData"), inside(code2));
    }
    function continuationStart(code2) {
      return effects.enter("lineEnding"), effects.consume(code2), effects.exit("lineEnding"), lineStart;
    }
...
function tokenizeNextBlank(effects, ok3, nok) {
  return start2;
  function start2(code2) {
    return effects.enter("lineEndingBlank"), effects.consume(code2), effects.exit("lineEndingBlank"), effects.attempt(blankLine, ok3, nok);
  }
}
...
function syntaxExtension(all2, extension2) {
  ...
  for (hook in extension2) {
    let left = (hasOwnProperty2.call(all2, hook) ? all2[hook] : void 0) || (all2[hook] = {}), right = extension2[hook], code2;
    if (right)
      for (code2 in right) {
...
function remarkMdx(options) {
  let self2 = (
    /** @type {Processor} */
    this
  ), settings = options || emptyOptions, data2 = self2.data(), micromarkExtensions = data2.micromarkExtensions || (data2.micromarkExtensions = []), fromMarkdownExtensions = data2.fromMarkdownExtensions || (data2.fromMarkdownExtensions = []), toMarkdownExtensions = data2.toMarkdownExtensions || (data2.toMarkdownExtensions = []);
...
function toString2(value, options) {
  let settings = options || emptyOptions2, includeImageAlt = typeof settings.includeImageAlt == "boolean" ? settings.includeImageAlt : !0, includeHtml = typeof settings.includeHtml == "boolean" ? settings.includeHtml : !0;
...
function normalizeUri(value) {
  ...
  for (; ++index2 < value.length; ) {
    ...
    replace && (result.push(value.slice(start2, index2), encodeURIComponent(replace)), start2 = index2 + skip + 1, replace = ""), skip && (index2 += skip, skip = 0);
  }
...
function initializeContent(effects) {
  ...
  function lineStart(code2) {
    ...
    return previous2 && (previous2.next = token), previous2 = token, data2(code2);
  }
  function data2(code2) {
    if (code2 === null) {
    ...
    return markdownLineEnding(code2) ? (effects.consume(code2), effects.exit("chunkText"), lineStart) : (effects.consume(code2), data2);
  }
}
var document = {
...
function initializeFactory(field) {
  ...
  function initializeText(effects) {
    let self2 = this, constructs2 = this.parser.constructs[field], text4 = effects.attempt(constructs2, start2, notText);
    return start2;
    function start2(code2) {
    ...
    function notText(code2) {
      ...
      return effects.enter("data"), effects.consume(code2), data2;
    }
    function data2(code2) {
      return atBreak(code2) ? (effects.exit("data"), text4(code2)) : (effects.consume(code2), data2);
    }
    function atBreak(code2) {
      if (code2 === null)
        return !0;
  ...
  function constructFactory(onreturn, fields) {
    ...
    function hook(constructs2, returnState, bogusState) {
      ...
      function handleMapOfConstructs(map) {
        return start2;
        function start2(code2) {
          let def = code2 !== null && map[code2], all2 = code2 !== null && map.null, list3 = [
      ...
      function handleConstruct(construct) {
        return start2;
        function start2(code2) {
          return info = store(), currentConstruct = construct, construct.partial || (context.currentConstruct = construct), construct.name && context.parser.constructs.disable.null.includes(construct.name) ? nok(code2) : construct.tokenize.call(
...
function compiler(options) {
  ...
  function prepareList(events, start2, length) {
    let index2 = start2 - 1, containerBalance = -1, listSpread = !1, listItem3, lineIndex, firstBlankLineIndex, atMarker;
    for (; ++index2 <= length; ) {
      let event = events[index2];
      switch (event[1].type) {
        case "listUnordered":
  ...
  function onexitcodefencedfenceinfo() {
    let data22 = this.resume(), node2 = this.stack[this.stack.length - 1];
    node2.lang = data22;
  }
  ...
  function onexitcharacterreferencevalue(token) {
    let data22 = this.sliceSerialize(token), type = this.data.characterReferenceType, value;
    type ? (value = decodeNumericCharacterReference(
      data22,
      type === "characterReferenceMarkerNumeric" ? 10 : 16
    ), this.data.characterReferenceType = void 0) : value = decodeNamedCharacterReference(data22);
    let tail = this.stack.pop();
...
function revert(state, node2) {
  ...
  let contents = state.all(node2), head = contents[0];
...
function imageReference(state, node2) {
  let id = String(node2.identifier).toUpperCase(), def = state.definitionById.get(id);
  if (!def)
...
function inlineCode(state, node2) {
  let text4 = { type: "text", value: node2.value.replace(/\r?\n|\r/g, " ") };
...
function table(state, node2) {
  ...
  if (rows.length > 0) {
    ...
    }, start2 = pointStart(node2.children[1]), end = pointEnd(node2.children[node2.children.length - 1]);
    start2 && end && (body.position = { start: start2, end }), tableContent.push(body);
  }
  let result = {
    type: "element",
    tagName: "table",
...
var VOID = -1, PRIMITIVE = 0, ARRAY = 1, OBJECT = 2, DATE = 3, REGEXP = 4, MAP = 5, SET = 6, ERROR = 7, BIGINT = 8, env = typeof self == "object" ? self : globalThis, deserializer = ($, _) => {
  let as = (out, index2) => ($.set(index2, out), out), unpair = (index2) => {
    if ($.has(index2))
      return $.get(index2);
    ...
    switch (type) {
      ...
      case MAP: {
        let map = as(/* @__PURE__ */ new Map(), index2);
        for (let [key, index3] of value)
  ...
  return unpair;
}, deserialize = (serialized) => deserializer(/* @__PURE__ */ new Map(), serialized)(0), EMPTY = "", { toString: toString22 } = {}, { keys } = Object, typeOf = (value) => {
  let type = typeof value;
  if (type !== "object" || !value)
    return [PRIMITIVE, type];
  let asString = toString22.call(value).slice(8, -1);
  switch (asString) {
    case "Array":
      return [ARRAY, EMPTY];
    case "Object":
  ...
  let as = (out, value) => {
    ...
    return $.set(value, index2), index2;
  }, pair = (value) => {
    if ($.has(value))
      return $.get(value);
...
function footer(state) {
  let clobberPrefix = typeof state.options.clobberPrefix == "string" ? state.options.clobberPrefix : "user-content-", footnoteBackContent = state.options.footnoteBackContent || defaultFootnoteBackContent, footnoteBackLabel = state.options.footnoteBackLabel || defaultFootnoteBackLabel, footnoteLabel = state.options.footnoteLabel || "Footnotes", footnoteLabelTagName = state.options.footnoteLabelTagName || "h2", footnoteLabelProperties = state.options.footnoteLabelProperties || {
    className: ["sr-only"]
  }, listItems = [], referenceIndex = -1;
  for (; ++referenceIndex < state.footnoteOrder.length; ) {
    let def = state.footnoteById.get(state.footnoteOrder[referenceIndex]);
    if (!def)
      continue;
    let content3 = state.all(def), id = String(def.identifier).toUpperCase(), safeId = normalizeUri(id.toLowerCase()), rereferenceIndex = 0, backReferences = [], counts = state.footnoteCounts.get(id);
    ...
    state.patch(def, listItem2), listItems.push(listItem2);
  }
...
var empty22 = [], CONTINUE2 = !0, EXIT2 = !1, SKIP2 = "skip";
function visitParents(tree, test, visitor, reverse) {
  ...
  function factory(node2, index2, parents) {
    ...
    return visit3;
    function visit3() {
      let result = empty22, subresult, offset2, grandparents;
      if ((!test || is2(node2, index2, parents[parents.length - 1] || void 0)) && (result = toResult2(visitor(node2, parents)), result[0] === EXIT2))
        return result;
      if ("children" in node2 && node2.children) {
        ...
        if (nodeAsParent.children && result[0] !== SKIP2)
          for (offset2 = (reverse ? nodeAsParent.children.length : -1) + step, grandparents = parents.concat(nodeAsParent); offset2 > -1 && offset2 < nodeAsParent.children.length; ) {
            let child = nodeAsParent.children[offset2];
            if (subresult = factory(child, offset2, grandparents)(), subresult[0] === EXIT2)
              return subresult;
...
function toResult2(value) {
  return Array.isArray(value) ? value : typeof value == "number" ? [CONTINUE2, value] : value == null ? empty22 : [value];
}
...
var own4 = {}.hasOwnProperty, emptyOptions3 = {};
function createState(tree, options) {
  let settings = options || emptyOptions3, definitionById = /* @__PURE__ */ new Map(), footnoteById = /* @__PURE__ */ new Map(), footnoteCounts = /* @__PURE__ */ new Map(), handlers2 = { ...handlers, ...settings.handlers }, state = {
    all: all2,
    applyData,
    definitionById,
    footnoteById,
  ...
  return visit2(tree, function(node2) {
    if (node2.type === "definition" || node2.type === "footnoteDefinition") {
      let map = node2.type === "definition" ? definitionById : footnoteById, id = String(node2.identifier).toUpperCase();
      map.has(id) || map.set(id, node2);
    }
  }), state;
  function one2(node2, parent) {
    let type = node2.type, handle = state.handlers[type];
    if (own4.call(state.handlers, type) && handle)
      return handle(state, node2, parent);
    if (state.options.passThrough && state.options.passThrough.includes(type)) {
      if ("children" in node2) {
        let { children, ...shallow } = node2, result = esm_default(shallow);
  ...
  function all2(parent) {
    ...
    if ("children" in parent) {
      ...
      for (; ++index2 < nodes.length; ) {
        let result = state.one(nodes[index2], parent);
        if (result) {
  ...
  if (from && from.data) {
    ...
    if (typeof hName == "string")
      if (result.type === "element")
        ...
        let children = "children" in result ? result.children : [result];
        result = { type: "element", tagName: hName, properties: {}, children };
      }
    result.type === "element" && hProperties && Object.assign(result.properties, esm_default(hProperties)), "children" in result && result.children && hChildren !== null && hChildren !== void 0 && (result.children = hChildren);
  }
  return result;
}
function defaultUnknownHandler(state, node2) {
  let data2 = node2.data || {}, result = "value" in node2 && !(own4.call(data2, "hProperties") || own4.call(data2, "hChildren")) ? { type: "text", value: node2.value } : {
    type: "element",
    tagName: "div",
    properties: {},
    children: state.all(node2)
...
function toHast(tree, options) {
  let state = createState(tree, options), node2 = state.one(tree, void 0), foot = footer(state), result = Array.isArray(node2) ? { type: "root", children: node2 } : node2 || { type: "root", children: [] };
...
var CallableInstance = (
  ...
  (function(property) {
    ...
    for (let p of names) {
      let descriptor = Object.getOwnPropertyDescriptor(func, p);
      descriptor && Object.defineProperty(apply, p, descriptor);
    }
    return apply;
  })
), own5 = {}.hasOwnProperty, Processor = class extends CallableInstance {
  /**
   * Create a processor.
   */
  constructor() {
   ...
   */
  data(key, value) {
    return typeof key == "string" ? arguments.length === 2 ? (assertUnfrozen("data", this.frozen), this.namespace[key] = value, this) : own5.call(this.namespace, key) && this.namespace[key] || void 0 : key ? (assertUnfrozen("data", this.frozen), this.namespace = key, this) : this.namespace;
  }
  /**
   * Freeze a processor.
   *
  ...
  use(value, ...parameters) {
    ...
    function addPlugin(plugin, parameters2) {
      ...
      if (entryIndex === -1)
        ...
        isPlainObject(currentPrimary) && isPlainObject(primary) && (primary = (0, import_extend.default)(!0, currentPrimary, primary)), attachers[entryIndex] = [plugin, primary, ...rest];
      }
    }
  }
...
var WalkerBase = class {
  ...
  replace(parent, prop, index2, node2) {
    parent && prop && (index2 != null ? parent[prop][index2] = node2 : parent[prop] = node2);
  }
  /**
   * @template {Node} Parent
   * @param {Parent | null | undefined} parent
  ...
  remove(parent, prop, index2) {
    parent && prop && (index2 != null ? parent[prop].splice(index2, 1) : delete parent[prop]);
  }
  ...
  visit(node2, parent, prop, index2) {
    if (node2) {
      if (this.enter) {
        ...
        this.should_skip = !1, this.should_remove = !1, this.replacement = null, this.enter.call(this.context, node2, parent, prop, index2), this.replacement && (node2 = this.replacement, this.replace(parent, prop, index2, node2)), this.should_remove && this.remove(parent, prop, index2);
        let skipped = this.should_skip, removed = this.should_remove;
        if (this.should_skip = _should_skip, this.should_remove = _should_remove, this.replacement = _replacement, skipped)
          return node2;
        if (removed)
          return null;
      }
        ...
        if (value && typeof value == "object")
          if (Array.isArray(value)) {
            ...
            for (let i = 0; i < nodes.length; i += 1) {
              let item = nodes[i];
              isNode(item) && (this.visit(item, node2, key, i) || i--);
            }
          } else isNode(value) && this.visit(value, node2, key, null);
      }
      if (this.leave) {
        let _replacement = this.replacement, _should_remove = this.should_remove;
        this.replacement = null, this.should_remove = !1, this.leave.call(this.context, node2, parent, prop, index2), this.replacement && (node2 = this.replacement, this.replace(parent, prop, index2, node2)), this.should_remove && this.remove(parent, prop, index2);
        let removed = this.should_remove;
        if (this.replacement = _replacement, this.should_remove = _should_remove, removed)
          return null;
      }
    }
    return node2;
...
function analyze(expression) {
  let map = /* @__PURE__ */ new WeakMap(), globals = /* @__PURE__ */ new Map(), scope = new Scope3(null, !1), references = [], current_scope = scope;
  walk(expression, {
    enter(node2, parent) {
      switch (node2.type) {
        case "Identifier":
          parent && is_reference(node2, parent) && references.push([current_scope, node2]);
          break;
        ...
        case "ArrowFunctionExpression":
          node2.type === "FunctionDeclaration" ? (node2.id && current_scope.declarations.set(node2.id.name, node2), map.set(node2, current_scope = new Scope3(current_scope, !1))) : (map.set(node2, current_scope = new Scope3(current_scope, !1)), node2.type === "FunctionExpression" && node2.id && current_scope.declarations.set(node2.id.name, node2)), node2.params.forEach((param) => {
        ...
        case "ForInStatement":
        case "ForOfStatement":
          map.set(node2, current_scope = new Scope3(current_scope, !0));
          break;
        case "BlockStatement":
          map.set(node2, current_scope = new Scope3(current_scope, !0));
          break;
        ...
        case "VariableDeclaration":
          ...
          break;
        case "CatchClause":
          map.set(node2, current_scope = new Scope3(current_scope, !0)), node2.param && extract_names(node2.param).forEach((name2) => {
            node2.param && current_scope.declarations.set(name2, node2.param);
          });
          break;
      }
    },
    leave(node2) {
      map.has(node2) && current_scope !== null && current_scope.parent && (current_scope = current_scope.parent);
    }
  });
  for (let i = references.length - 1; i >= 0; --i) {
    let [scope2, reference] = references[i];
    scope2.references.has(reference.name) || add_reference(scope2, reference.name), scope2.find_owner(reference.name) || globals.set(reference.name, reference);
  }
  return { map, scope, globals };
}
function add_reference(scope, name2) {
  scope.references.add(name2), scope.parent && add_reference(scope.parent, name2);
}
var Scope3 = class {
  /**
  ...
  constructor(parent, block) {
    this.parent = parent, this.block = block, this.declarations = /* @__PURE__ */ new Map(), this.initialised_declarations = /* @__PURE__ */ new Set(), this.references = /* @__PURE__ */ new Set();
  }
  /**
  ...
  add_declaration(node2) {
    if (node2.type === "VariableDeclaration")
      if (node2.kind === "var" && this.block && this.parent)
        ...
        let handle_declarator = (declarator) => {
          extract_names(declarator.id).forEach((name2) => {
            this.declarations.set(name2, node2), declarator.init && this.initialised_declarations.add(name2);
          });
        };
        node2.declarations.forEach(handle_declarator);
      }
    else node2.id && this.declarations.set(node2.id.name, node2);
  }
  /**
   * @param {string} name
   * @returns {Scope | null}
   */
  find_owner(name2) {
    return this.declarations.has(name2) ? this : this.parent && this.parent.find_owner(name2);
  }
  /**
   * @param {string} name
   * @returns {boolean}
...
function extract_identifiers(param, nodes = []) {
  switch (param.type) {
    ...
    case "MemberExpression":
      let object = param;
      for (; object.type === "MemberExpression"; )
        object = /** @type {any} */
        object.object;
      nodes.push(
        /** @type {any} */
    ...
    case "ObjectPattern":
      let handle_prop = (prop) => {
        prop.type === "RestElement" ? extract_identifiers(prop.argument, nodes) : extract_identifiers(prop.value, nodes);
      };
      param.properties.forEach(handle_prop);
      break;
    case "ArrayPattern":
      let handle_element = (element) => {
        element && extract_identifiers(element, nodes);
      };
      param.elements.forEach((element) => {
        element && handle_element(element);
      });
      break;
    case "RestElement":
...
function create(from, to) {
  let fields = ["start", "end", "loc", "range", "comments"], index2 = -1;
  for (; ++index2 < fields.length; ) {
    let field = fields[index2];
    field in from && (to[field] = from[field]);
  }
}
...
function specifiersToDeclarations(specifiers, init) {
  ...
  for (; ++index2 < specifiers.length; ) {
    let specifier = specifiers[index2];
    specifier.type === "ImportNamespaceSpecifier" ? importNamespaceSpecifier = specifier : otherSpecifiers.push(specifier);
  }
  if (importNamespaceSpecifier) {
    ...
    create(importNamespaceSpecifier, declarator), declarations.push(declarator);
  }
  return declarations.push({
    ...
    id: {
      ...
      properties: otherSpecifiers.map(function(specifier) {
        let key = specifier.type === "ImportSpecifier" ? specifier.imported : specifier.type === "ExportSpecifier" ? specifier.exported : { type: "Identifier", name: "default" }, value = specifier.local;
        specifier.type === "ExportSpecifier" && (value = key, key = specifier.local);
        let property = {
          type: "Property",
          kind: "init",
        ...
        return create(specifier, property), property;
      })
    },
...
function toIdOrMemberExpression(ids) {
  ...
  for (; ++index2 < ids.length; ) {
    let name2 = ids[index2], id = typeof name2 == "string" && name(name2) ? { type: "Identifier", name: name2 } : { type: "Literal", value: name2 };
    object = object ? {
...
function toJsxIdOrMemberExpression(ids) {
  let index2 = -1, object;
  for (; ++index2 < ids.length; ) {
    let name2 = ids[index2];
    typeof name2 == "string" && name(name2, { jsx: !0 }), "" + name2;
    let id = { type: "JSXIdentifier", name: name2 };
...
function recmaDocument(options) {
  ...
  return function(tree, file) {
        ...
        child.specifiers = child.specifiers.filter(function(specifier) {
          if (specifier.exported.name === "default") {
            ...
            if (specifier.local.name === "default")
              ...
              create(specifier.local, importSpecifier), specifiers.push(importSpecifier);
            }
            let from = { type: "Literal", value: source.value };
            create(source, from);
            let declaration = {
              type: "ImportDeclaration",
              specifiers,
            ...
            return create(specifier, declaration), handleEsm(declaration), !1;
          }
          return !0;
        }), child.specifiers.length > 0 && handleExport(child);
      } else child.type === "ExportNamedDeclaration" || child.type === "ExportAllDeclaration" ? handleExport(child) : child.type === "ImportDeclaration" ? handleEsm(child) : child.type === "ExpressionStatement" && (child.expression.type === "JSXElement" || // @ts-expect-error: `estree-jsx` does not register `JSXFragment` as an expression.
      child.expression.type === "JSXFragment") ? (content3 = !0, replacement.push(
        ...createMdxContent(child.expression, outputFormat, !!layout)
      )) : replacement.push(child);
    content3 || replacement.push(
      ...createMdxContent(void 0, outputFormat, !!layout)
    ), exportedIdentifiers.push(["MDXContent", "default"]), outputFormat === "function-body" && replacement.push({
    ...
    let usesImportMetaUrlVariable = !1, usesResolveDynamicHelper = !1;
    (baseHref || outputFormat === "function-body") && walk(tree, {
      enter(node2) {
        if ((node2.type === "ExportAllDeclaration" || node2.type === "ExportNamedDeclaration" || node2.type === "ImportDeclaration") && node2.source) {
          ...
          try {
            ...
            (value.startsWith("/") || value.startsWith("./") || value.startsWith("../")) && (value = new URL(value, baseHref).href);
          }
          let replacement2 = { type: "Literal", value };
          create(node2.source, replacement2), node2.source = replacement2;
          return;
        }
        if (node2.type === "ImportExpression") {
          usesResolveDynamicHelper = !0;
          let replacement2 = {
            type: "CallExpression",
        ...
        if (node2.type === "MemberExpression" && "object" in node2 && node2.object.type === "MetaProperty" && node2.property.type === "Identifier" && node2.object.meta.name === "import" && node2.object.property.name === "meta" && node2.property.name === "url") {
          usesImportMetaUrlVariable = !0;
          let replacement2 = { type: "Identifier", name: "_importMetaUrl" };
          create(node2, replacement2), this.replace(replacement2);
        }
      }
    }), usesResolveDynamicHelper && (baseHref || (usesImportMetaUrlVariable = !0), tree.body.push(
      resolveDynamicMdxSpecifier(
        baseHref ? { type: "Literal", value: baseHref } : { type: "Identifier", name: "_importMetaUrl" }
      )
    )), usesImportMetaUrlVariable && tree.body.unshift(...createImportMetaUrlVariable());
    function handleExport(node2) {
      if (node2.type === "ExportNamedDeclaration") {
        node2.declaration && exportedIdentifiers.push(
    ...
    function handleEsm(node2) {
      ...
      if (outputFormat === "function-body")
        if (
          ...
          node2.source;
          let argument = { type: "ImportExpression", source: node2.source };
          create(node2, argument), init = { type: "AwaitExpression", argument }, (node2.type === "ImportDeclaration" || node2.type === "ExportNamedDeclaration") && node2.specifiers.length === 0 ? replace = { type: "ExpressionStatement", expression: init } : replace = {
            type: "VariableDeclaration",
            kind: "const",
          ...
          let declarators = node2.specifiers.filter(function(specifier) {
        ...
        replace = node2;
      replace && replacement.push(replace);
    }
  };
  function createMdxContent(content3, outputFormat2, hasInternalLayout) {
    ...
    argument.type === "JSXFragment" && argument.children.length === 1 && argument.children[0].type === "JSXElement" && (argument = argument.children[0]);
    let awaitExpression = !1;
    walk(argument, {
      enter(node2) {
        if (node2.type === "ArrowFunctionExpression" || node2.type === "FunctionDeclaration" || node2.type === "FunctionExpression")
          return this.skip();
        (node2.type === "AwaitExpression" || /* c8 ignore next 2 -- can only occur in a function (which then can
        * only be async, so skipped it) */
        node2.type === "ForOfStatement" && node2.await) && (awaitExpression = !0);
      }
    });
    let declaration = {
  ...
  walk(tree, {
    enter(node2) {
      if (node2.type === "Program") {
        let comments = node2.comments || [], index2 = -1;
        for (; ++index2 < comments.length; ) {
          regex.lastIndex = 0;
          let match = regex.exec(comments[index2].value);
          for (; match; )
            annotations[match[1]] = match[2], match = regex.exec(comments[index2].value);
        }
        if (annotations.jsxRuntime)
          if (annotations.jsxRuntime === "automatic") {
            if (automatic = !0, annotations.jsx)
            ...
            if (automatic = !1, annotations.jsxImportSource)
              throw new Error(
    ...
    leave(node2) {
      ...
      if (node2.type !== "JSXElement" && node2.type !== "JSXFragment")
        return;
      let children = [], index2 = -1;
      for (; ++index2 < node2.children.length; ) {
        let child = node2.children[index2];
        if (child.type === "JSXExpressionContainer")
          child.expression.type !== "JSXEmptyExpression" && children.push(child.expression);
        else if (child.type === "JSXText") {
          let value = child.value.replace(/\t/g, " ").replace(/ *(\r?\n|\r) */g, `
`).replace(/\n+/g, `
`).replace(/\n+$/, "").replace(/^\n+/, "").replace(/\n/g, " ");
          if (value) {
            let text4 = { type: "Literal", value };
            create2(child, text4), children.push(text4);
          }
        } else
          child.type !== "JSXElement" && child.type !== "JSXFragment" && child.type, children.push(child);
      }
      let name2, fields = [], parameters = [], key;
      if (node2.type === "JSXElement") {
        if (name2 = toIdentifier(node2.openingElement.name), name2.type === "Identifier" && /^[a-z]/.test(name2.name)) {
          let next = { type: "Literal", value: name2.name };
          create2(name2, next), name2 = next;
        }
        let spread, attributes = node2.openingElement.attributes, index3 = -1;
        for (; ++index3 < attributes.length; ) {
          let attribute = attributes[index3];
          if (attribute.type === "JSXSpreadAttribute")
            attribute.argument.type === "ObjectExpression" ? fields.push(...attribute.argument.properties) : fields.push({ type: "SpreadElement", argument: attribute.argument }), spread = !0;
          else {
            let prop = toProperty(attribute);
            if (automatic && prop.key.type === "Identifier" && prop.key.name === "key") {
              ...
              value.type !== "AssignmentPattern" && value.type !== "ArrayPattern" && value.type !== "ObjectPattern" && value.type, key = value;
            } else
              fields.push(prop);
          }
        }
      } else automatic ? (imports.fragment = !0, name2 = { type: "Identifier", name: "_Fragment" }) : name2 = toMemberExpression(
      ...
      let callee;
      if (automatic) {
        parameters.push({ type: "ObjectExpression", properties: fields }), key ? parameters.push(key) : config.development && parameters.push({ type: "Identifier", name: "undefined" });
        let isStaticChildren = children.length > 1;
        if (config.development) {
          imports.jsxDEV = !0, callee = {
            type: "Identifier",
            name: "_jsxDEV"
          ...
          node2.loc && source.properties.push(
            {
              type: "Property",
              method: !1,
              shorthand: !1,
        ...
        } else isStaticChildren ? (imports.jsxs = !0, callee = { type: "Identifier", name: "_jsxs" }) : (imports.jsx = !0, callee = { type: "Identifier", name: "_jsx" });
      } else
        fields.length > 0 ? parameters.unshift({ type: "ObjectExpression", properties: fields }) : parameters.length > 0 && parameters.unshift({ type: "Literal", value: null }), callee = toMemberExpression(
      ...
      create2(node2, call), this.replace(call);
    }
  });
}
function toProperty(node2) {
  ...
  if (node2.value)
    if (node2.value.type === "JSXExpressionContainer") {
      let valueExpression = node2.value.expression;
      valueExpression.type, value = valueExpression;
    } else {
      let nodeValue = node2.value;
      nodeValue.type !== "JSXElement" && nodeValue.type, value = nodeValue, delete value.raw;
    }
  else
  ...
  return create2(node2, replacement), replacement;
}
function toIdentifier(node2) {
  let replace;
  if (node2.type === "JSXMemberExpression") {
    let id = toIdentifier(node2.property);
    replace = {
      type: "MemberExpression",
  ...
  } else node2.type === "JSXNamespacedName" ? replace = {
    type: "Literal",
  ...
  return create2(node2, replace), replace;
}
function toMemberExpression(id) {
  let identifiers = id.split("."), index2 = -1, result;
  for (; ++index2 < identifiers.length; ) {
    let prop = name(identifiers[index2]) ? { type: "Identifier", name: identifiers[index2] } : { type: "Literal", value: identifiers[index2] };
    result = result ? {
      type: "MemberExpression",
      object: result,
      property: prop,
...
function recmaJsxBuild(options) {
  let { development, outputFormat } = options || {};
  return function(tree, file) {
    if (buildJsx(tree, { development, filePath: file.history[0] }), tree.comments && tree.comments[0].type === "Block" && tree.comments[0].data && tree.comments[0].data._mdxIsPragmaComment && tree.comments.shift(), outputFormat === "function-body") {
      let index2 = 0;
      for (; index2 < tree.body.length; ) {
        let child = tree.body[index2];
        if ("directive" in child && child.directive)
          index2++;
        else
          break;
      }
      let declaration = tree.body[index2];
      declaration && declaration.type === "ImportDeclaration" && typeof declaration.source.value == "string" && /\/jsx-(dev-)?runtime$/.test(declaration.source.value) && (tree.body[index2] = {
        type: "VariableDeclaration",
        kind: "const",
...
function recmaJsxRewrite(options) {
  ...
  return function(tree, file) {
    let scopeInfo = analyze(tree), fnStack = [], importProvider = !1, createErrorHelper = !1, currentScope;
    if (walk(tree, {
      enter(node2) {
        let newScope = (
        ...
        (node2.type === "FunctionDeclaration" || node2.type === "FunctionExpression" || node2.type === "ArrowFunctionExpression") && (fnStack.push({
          components: [],
          ...
          references: {},
          tags: []
        }), isNamedFunction(node2, "MDXContent") && newScope && !inScope(newScope, "MDXLayout") && fnStack[0].components.push("MDXLayout"));
        let fnScope = fnStack[0];
        if (!(!fnScope || !isNamedFunction(fnScope.node, "_createMdxContent") && !providerImportSource) && (newScope && (newScope.node = node2, currentScope = newScope), currentScope && node2.type === "JSXElement")) {
          let name2 = node2.openingElement.name;
          if (name2.type === "JSXMemberExpression") {
            let ids = [];
            for (; name2.type === "JSXMemberExpression"; )
              ids.unshift(name2.property.name), name2 = name2.object;
            ids.unshift(name2.name);
            let fullId = ids.join("."), id = name2.name, isInScope = inScope(currentScope, id);
            if (!Object.hasOwn(fnScope.references, fullId)) {
              let parentScope = (
                /** @type {Scope | undefined} */
                currentScope.parent
              );
              (!isInScope || // If the parent scope is `_createMdxContent`, then this
              // references a component we can add a check statement for.
              parentScope && parentScope.node.type === "FunctionDeclaration" && isNamedFunction(parentScope.node, "_createMdxContent")) && (fnScope.references[fullId] = { component: !0, node: node2 });
            }
            !fnScope.objects.includes(id) && !isInScope && fnScope.objects.push(id);
          } else if (name2.type !== "JSXNamespacedName") {
            if (name(name2.name) && !/^[a-z]/.test(name2.name)) {
              let id = name2.name;
              inScope(currentScope, id) || (id !== "MDXLayout" && !Object.hasOwn(fnScope.references, id) && (fnScope.references[id] = { component: !0, node: node2 }), fnScope.components.includes(id) || fnScope.components.push(id));
            } else if (!(node2.data && node2.data._mdxExplicitJsx)) {
              let id = name2.name;
              fnScope.tags.includes(id) || fnScope.tags.push(id);
              let jsxIdExpression = ["_components", id];
              if (name(id) === !1) {
                let invalidComponentName = fnScope.idToInvalidComponentName.get(id);
                invalidComponentName === void 0 && (invalidComponentName = `_component${fnScope.idToInvalidComponentName.size}`, fnScope.idToInvalidComponentName.set(id, invalidComponentName)), jsxIdExpression = [invalidComponentName];
              }
              node2.openingElement.name = toJsxIdOrMemberExpression(jsxIdExpression), node2.closingElement && (node2.closingElement.name = toJsxIdOrMemberExpression(jsxIdExpression));
            }
          }
        }
      },
      leave(node2) {
        let defaults = [], actual = [], parameters = [], declarations = [];
        if (currentScope && currentScope.node === node2 && (currentScope = /** @type {Scope} */
        currentScope.parent), node2.type === "FunctionDeclaration" || node2.type === "FunctionExpression" || node2.type === "ArrowFunctionExpression") {
          let fn = node2, scope = fnStack[fnStack.length - 1], name2;
          ...
          for (name2 of scope.objects)
            actual.includes(name2) || actual.push(name2);
          actual.sort();
          let statements = [];
          if (defaults.length > 0 || actual.length > 0 || scope.idToInvalidComponentName.size > 0) {
            if (providerImportSource && (importProvider = !0, parameters.push({
              type: "CallExpression",
              callee: { type: "Identifier", name: "_provideComponents" },
              arguments: [],
              optional: !1
            })), (isNamedFunction(scope.node, "MDXContent") || isNamedFunction(scope.node, "_createMdxContent")) && parameters.push(toIdOrMemberExpression(["props", "components"])), defaults.length > 0 || parameters.length > 1)
            ...
            if (actual.length > 0 && (componentsPattern = {
              type: "ObjectPattern",
            ...
            }), scope.tags.length > 0 && (declarations.push({
              type: "VariableDeclarator",
              id: { type: "Identifier", name: "_components" },
              init: componentsInit
            }), componentsInit = { type: "Identifier", name: "_components" }), isNamedFunction(scope.node, "_createMdxContent"))
              for (let [id, componentName] of [
            ...
            componentsPattern && declarations.push({
              type: "VariableDeclarator",
              id: componentsPattern,
              init: componentsInit
            }), declarations.length > 0 && statements.push({
              type: "VariableDeclaration",
              kind: "const",
              declarations
            });
          ...
          for (key in scope.references)
            if (Object.hasOwn(scope.references, key)) {
              let parts = key.split("."), index3 = 0;
              for (; ++index3 < parts.length; ) {
                let partial = parts.slice(0, index3).join(".");
                Object.hasOwn(scope.references, partial) || (scope.references[partial] = {
                  component: !1,
                  node: scope.references[key].node
                });
              }
            }
          let references = Object.keys(scope.references).sort(), index2 = -1;
          for (; ++index2 < references.length; ) {
            let id = references[index2], info = scope.references[id], place = stringifyPosition(positionFromEstree(info.node)), parameters2 = [
              { type: "Literal", value: id },
              { type: "Literal", value: info.component }
            ];
            createErrorHelper = !0, development && place && parameters2.push({ type: "Literal", value: place }), statements.push({
              type: "IfStatement",
              test: {
                type: "UnaryExpression",
                operator: "!",
          ...
          statements.length > 0 && (fn.body.type !== "BlockStatement" && (fn.body = {
            type: "BlockStatement",
            body: [{ type: "ReturnStatement", argument: fn.body }]
          }), fn.body.body.unshift(...statements)), fnStack.pop();
        }
      }
    }), importProvider && providerImportSource && tree.body.unshift(
      ...
      }), parameters.push({ type: "Identifier", name: "place" })), tree.body.push({
        type: "FunctionDeclaration",
    ...
    outputFormat === "function-body" && tree.body.unshift({
      type: "ExpressionStatement",
...
function isNamedFunction(node2, name2) {
  return !!(node2 && "id" in node2 && node2.id && node2.id.name === name2);
}
...
function expressionNeedsParenthesis(state, node2, parentNode, isRightHand) {
  let nodePrecedence = state.expressionsPrecedence[node2.type];
  if (nodePrecedence === NEEDS_PARENTHESES)
    return !0;
  let parentNodePrecedence = state.expressionsPrecedence[parentNode.type];
  return nodePrecedence !== parentNodePrecedence ? !isRightHand && nodePrecedence === 15 && parentNodePrecedence === 14 && parentNode.operator === "**" || nodePrecedence < parentNodePrecedence : nodePrecedence !== 13 && nodePrecedence !== 14 ? !1 : node2.operator === "**" && parentNode.operator === "**" ? !isRightHand : nodePrecedence === 13 && parentNodePrecedence === 13 && (node2.operator === "??" || parentNode.operator === "??") ? !0 : isRightHand ? OPERATOR_PRECEDENCE[node2.operator] <= OPERATOR_PRECEDENCE[parentNode.operator] : OPERATOR_PRECEDENCE[node2.operator] < OPERATOR_PRECEDENCE[parentNode.operator];
}
function formatExpression(state, node2, parentNode, isRightHand) {
  let { generator } = state;
  expressionNeedsParenthesis(state, node2, parentNode, isRightHand) ? (state.write("("), generator[node2.type](node2, state), state.write(")")) : generator[node2.type](node2, state);
}
function reindent(state, text4, indent2, lineEnd) {
...
function formatComments(state, comments, indent2, lineEnd) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    state.write(indent2), comment.type[0] === "L" ? state.write("// " + comment.value.trim() + `
`, comment) : (state.write("/*"), reindent(state, comment.value, indent2, lineEnd), state.write("*/" + lineEnd));
  }
}
...
function formatVariableDeclaration(state, node2) {
  let { generator } = state, { declarations } = node2;
  state.write(node2.kind + " ");
  let { length } = declarations;
  if (length > 0) {
    generator.VariableDeclarator(declarations[0], state);
    for (let i = 1; i < length; i++)
      state.write(", "), generator.VariableDeclarator(declarations[i], state);
  }
}
var ForInStatement, FunctionDeclaration, RestElement, BinaryExpression, ArrayExpression, BlockStatement, GENERATOR = {
  ...
  */
  ...
  BlockStatement: BlockStatement = function(node2, state) {
    let indent2 = state.indent.repeat(state.indentLevel++), { lineEnd, writeComments } = state, statementIndent = indent2 + state.indent;
    state.write("{");
    let statements = node2.body;
    if (statements != null && statements.length > 0) {
      ...
      writeComments && node2.comments != null && (state.write(lineEnd), formatComments(state, node2.comments, statementIndent, lineEnd), state.write(indent2));
    writeComments && node2.trailingComments != null && formatComments(state, node2.trailingComments, statementIndent, lineEnd), state.write("}"), state.indentLevel--;
  },
  ClassBody: BlockStatement,
  StaticBlock(node2, state) {
    state.write("static "), this.BlockStatement(node2, state);
  },
  EmptyStatement(node2, state) {
    state.write(";");
  },
  ExpressionStatement(node2, state) {
    let precedence = state.expressionsPrecedence[node2.expression.type];
    precedence === NEEDS_PARENTHESES || precedence === 3 && node2.expression.left.type[0] === "O" ? (state.write("("), this[node2.expression.type](node2.expression, state), state.write(")")) : this[node2.expression.type](node2.expression, state), state.write(";");
  },
  IfStatement(node2, state) {
    state.write("if ("), this[node2.test.type](node2.test, state), state.write(") "), this[node2.consequent.type](node2.consequent, state), node2.alternate != null && (state.write(" else "), this[node2.alternate.type](node2.alternate, state));
  },
  LabeledStatement(node2, state) {
    this[node2.label.type](node2.label, state), state.write(": "), this[node2.body.type](node2.body, state);
  },
  BreakStatement(node2, state) {
    state.write("break"), node2.label != null && (state.write(" "), this[node2.label.type](node2.label, state)), state.write(";");
  },
  ContinueStatement(node2, state) {
    state.write("continue"), node2.label != null && (state.write(" "), this[node2.label.type](node2.label, state)), state.write(";");
  },
  WithStatement(node2, state) {
    state.write("with ("), this[node2.object.type](node2.object, state), state.write(") "), this[node2.body.type](node2.body, state);
  },
  SwitchStatement(node2, state) {
    ...
    for (let i = 0; i < occurencesCount; i++) {
      ...
      for (let i2 = 0; i2 < consequentCount; i2++) {
        let statement = consequent[i2];
        writeComments && statement.comments != null && formatComments(state, statement.comments, statementIndent, lineEnd), state.write(statementIndent), this[statement.type](statement, state), state.write(lineEnd);
      }
    }
    state.indentLevel -= 2, state.write(indent2 + "}");
  },
  ReturnStatement(node2, state) {
    state.write("return"), node2.argument && (state.write(" "), this[node2.argument.type](node2.argument, state)), state.write(";");
  },
  ThrowStatement(node2, state) {
    state.write("throw "), this[node2.argument.type](node2.argument, state), state.write(";");
  },
  TryStatement(node2, state) {
    if (state.write("try "), this[node2.block.type](node2.block, state), node2.handler) {
      let { handler } = node2;
      handler.param == null ? state.write(" catch ") : (state.write(" catch ("), this[handler.param.type](handler.param, state), state.write(") ")), this[handler.body.type](handler.body, state);
    }
    node2.finalizer && (state.write(" finally "), this[node2.finalizer.type](node2.finalizer, state));
  },
  WhileStatement(node2, state) {
    state.write("while ("), this[node2.test.type](node2.test, state), state.write(") "), this[node2.body.type](node2.body, state);
  },
  DoWhileStatement(node2, state) {
    state.write("do "), this[node2.body.type](node2.body, state), state.write(" while ("), this[node2.test.type](node2.test, state), state.write(");");
  },
  ForStatement(node2, state) {
    if (state.write("for ("), node2.init != null) {
      let { init } = node2;
      init.type[0] === "V" ? formatVariableDeclaration(state, init) : this[init.type](init, state);
    }
    state.write("; "), node2.test && this[node2.test.type](node2.test, state), state.write("; "), node2.update && this[node2.update.type](node2.update, state), state.write(") "), this[node2.body.type](node2.body, state);
  },
  ForInStatement: ForInStatement = function(node2, state) {
    state.write(`for ${node2.await ? "await " : ""}(`);
    let { left } = node2;
    left.type[0] === "V" ? formatVariableDeclaration(state, left) : this[left.type](left, state), state.write(node2.type[3] === "I" ? " in " : " of "), this[node2.right.type](node2.right, state), state.write(") "), this[node2.body.type](node2.body, state);
  },
  ...
  FunctionDeclaration: FunctionDeclaration = function(node2, state) {
    state.write(
      ...
      node2
    ), formatSequence(state, node2.params), state.write(" "), this[node2.body.type](node2.body, state);
  },
  FunctionExpression: FunctionDeclaration,
  VariableDeclaration(node2, state) {
    formatVariableDeclaration(state, node2), state.write(";");
  },
  VariableDeclarator(node2, state) {
    this[node2.id.type](node2.id, state), node2.init != null && (state.write(" = "), this[node2.init.type](node2.init, state));
  },
  ClassDeclaration(node2, state) {
    if (state.write("class " + (node2.id ? `${node2.id.name} ` : ""), node2), node2.superClass) {
      state.write("extends ");
      let { superClass } = node2, { type } = superClass, precedence = state.expressionsPrecedence[type];
      (type[0] !== "C" || type[1] !== "l" || type[5] !== "E") && (precedence === NEEDS_PARENTHESES || precedence < state.expressionsPrecedence.ClassExpression) ? (state.write("("), this[node2.superClass.type](superClass, state), state.write(")")) : this[superClass.type](superClass, state), state.write(" ");
    }
    this.ClassBody(node2.body, state);
  },
  ImportDeclaration(node2, state) {
    state.write("import ");
    let { specifiers } = node2, { length } = specifiers, i = 0;
    if (length > 0) {
      for (; i < length; ) {
        i > 0 && state.write(", ");
        let specifier = specifiers[i], type = specifier.type[6];
        if (type === "D")
          state.write(specifier.local.name, specifier), i++;
        else if (type === "N")
          state.write("* as " + specifier.local.name, specifier), i++;
        else
          break;
      }
      if (i < length) {
        for (state.write("{"); ; ) {
          let specifier = specifiers[i], { name: name2 } = specifier.imported;
          if (state.write(name2, specifier), name2 !== specifier.local.name && state.write(" as " + specifier.local.name), ++i < length)
            state.write(", ");
          else
            break;
        }
    ...
    this.Literal(node2.source, state), state.write(";");
  },
  ImportExpression(node2, state) {
    state.write("import("), this[node2.source.type](node2.source, state), state.write(")");
  },
  ExportDefaultDeclaration(node2, state) {
    state.write("export default "), this[node2.declaration.type](node2.declaration, state), state.expressionsPrecedence[node2.declaration.type] != null && node2.declaration.type[0] !== "F" && state.write(";");
  },
  ExportNamedDeclaration(node2, state) {
    if (state.write("export "), node2.declaration)
      ...
      if (length > 0)
        for (let i = 0; ; ) {
          let specifier = specifiers[i], { name: name2 } = specifier.local;
          if (state.write(name2, specifier), name2 !== specifier.exported.name && state.write(" as " + specifier.exported.name), ++i < length)
            state.write(", ");
          else
            break;
        }
      state.write("}"), node2.source && (state.write(" from "), this.Literal(node2.source, state)), state.write(";");
    }
  },
  ExportAllDeclaration(node2, state) {
    node2.exported != null ? state.write("export * as " + node2.exported.name + " from ") : state.write("export * from "), this.Literal(node2.source, state), state.write(";");
  },
  MethodDefinition(node2, state) {
    node2.static && state.write("static ");
    let kind = node2.kind[0];
    (kind === "g" || kind === "s") && state.write(node2.kind + " "), node2.value.async && state.write("async "), node2.value.generator && state.write("*"), node2.computed ? (state.write("["), this[node2.key.type](node2.key, state), state.write("]")) : this[node2.key.type](node2.key, state), formatSequence(state, node2.value.params), state.write(" "), this[node2.value.body.type](node2.value.body, state);
  },
  ...
  ArrowFunctionExpression(node2, state) {
    ...
    let { params } = node2;
    params != null && (params.length === 1 && params[0].type[0] === "I" ? state.write(params[0].name, params[0]) : formatSequence(state, node2.params)), state.write(" => "), node2.body.type[0] === "O" ? (state.write("("), this.ObjectExpression(node2.body, state), state.write(")")) : this[node2.body.type](node2.body, state);
  },
  ...
  RestElement: RestElement = function(node2, state) {
    state.write("..."), this[node2.argument.type](node2.argument, state);
  },
  SpreadElement: RestElement,
  YieldExpression(node2, state) {
    state.write(node2.delegate ? "yield*" : "yield"), node2.argument && (state.write(" "), this[node2.argument.type](node2.argument, state));
  },
  AwaitExpression(node2, state) {
    state.write("await ", node2), formatExpression(state, node2.argument, node2);
  },
  TemplateLiteral(node2, state) {
    ...
    for (let i = 0; i < length; i++) {
      let expression = expressions[i], quasi2 = quasis[i];
    ...
    state.write(quasi.value.raw, quasi), state.write("`");
  },
  TemplateElement(node2, state) {
    state.write(node2.value.raw, node2);
  },
  TaggedTemplateExpression(node2, state) {
    formatExpression(state, node2.tag, node2), this[node2.quasi.type](node2.quasi, state);
  },
  ArrayExpression: ArrayExpression = function(node2, state) {
    if (state.write("["), node2.elements.length > 0) {
      ...
      for (let i = 0; ; ) {
        ...
        if (element != null && this[element.type](element, state), ++i < length)
          state.write(", ");
        else {
          element == null && state.write(", ");
          break;
        }
      }
    }
  ...
  ObjectExpression(node2, state) {
    let indent2 = state.indent.repeat(state.indentLevel++), { lineEnd, writeComments } = state, propertyIndent = indent2 + state.indent;
    if (state.write("{"), node2.properties.length > 0) {
      ...
      for (let i = 0; ; ) {
        ...
        if (writeComments && property.comments != null && formatComments(state, property.comments, propertyIndent, lineEnd), state.write(propertyIndent), this[property.type](property, state), ++i < length)
          state.write(comma);
        else
          break;
      }
      state.write(lineEnd), writeComments && node2.trailingComments != null && formatComments(state, node2.trailingComments, propertyIndent, lineEnd), state.write(indent2 + "}");
    } else writeComments ? node2.comments != null ? (state.write(lineEnd), formatComments(state, node2.comments, propertyIndent, lineEnd), node2.trailingComments != null && formatComments(state, node2.trailingComments, propertyIndent, lineEnd), state.write(indent2 + "}")) : node2.trailingComments != null ? (state.write(lineEnd), formatComments(state, node2.trailingComments, propertyIndent, lineEnd), state.write(indent2 + "}")) : state.write("}") : state.write("}");
    state.indentLevel--;
  },
  Property(node2, state) {
    node2.method || node2.kind[0] !== "i" ? this.MethodDefinition(node2, state) : (node2.shorthand || (node2.computed ? (state.write("["), this[node2.key.type](node2.key, state), state.write("]")) : this[node2.key.type](node2.key, state), state.write(": ")), this[node2.value.type](node2.value, state));
  },
  PropertyDefinition(node2, state) {
    if (node2.static && state.write("static "), node2.computed && state.write("["), this[node2.key.type](node2.key, state), node2.computed && state.write("]"), node2.value == null) {
      node2.key.type[0] !== "F" && state.write(";");
      return;
    }
    state.write(" = "), this[node2.value.type](node2.value, state), state.write(";");
  },
  ObjectPattern(node2, state) {
    if (state.write("{"), node2.properties.length > 0) {
      let { properties } = node2, { length } = properties;
      for (let i = 0; this[properties[i].type](properties[i], state), ++i < length; )
        state.write(", ");
    }
    state.write("}");
  },
  ...
  UnaryExpression(node2, state) {
    if (node2.prefix) {
      ...
      let needsParentheses = expressionNeedsParenthesis(state, argument, node2);
      !needsParentheses && (operator.length > 1 || type[0] === "U" && (type[1] === "n" || type[1] === "p") && argument.prefix && argument.operator[0] === operator && (operator === "+" || operator === "-")) && state.write(" "), needsParentheses ? (state.write(operator.length > 1 ? " (" : "("), this[type](argument, state), state.write(")")) : this[type](argument, state);
    } else
      this[node2.argument.type](node2.argument, state), state.write(node2.operator);
  },
  UpdateExpression(node2, state) {
    node2.prefix ? (state.write(node2.operator), this[node2.argument.type](node2.argument, state)) : (this[node2.argument.type](node2.argument, state), state.write(node2.operator));
  },
  AssignmentExpression(node2, state) {
    this[node2.left.type](node2.left, state), state.write(" " + node2.operator + " "), this[node2.right.type](node2.right, state);
  },
  AssignmentPattern(node2, state) {
    this[node2.left.type](node2.left, state), state.write(" = "), this[node2.right.type](node2.right, state);
  },
  BinaryExpression: BinaryExpression = function(node2, state) {
    let isIn = node2.operator === "in";
    isIn && state.write("("), formatExpression(state, node2.left, node2, !1), state.write(" " + node2.operator + " "), formatExpression(state, node2.right, node2, !0), isIn && state.write(")");
  },
  LogicalExpression: BinaryExpression,
  ConditionalExpression(node2, state) {
    let { test } = node2, precedence = state.expressionsPrecedence[test.type];
    precedence === NEEDS_PARENTHESES || precedence <= state.expressionsPrecedence.ConditionalExpression ? (state.write("("), this[test.type](test, state), state.write(")")) : this[test.type](test, state), state.write(" ? "), this[node2.consequent.type](node2.consequent, state), state.write(" : "), this[node2.alternate.type](node2.alternate, state);
  },
  NewExpression(node2, state) {
    state.write("new ");
    let precedence = state.expressionsPrecedence[node2.callee.type];
    precedence === NEEDS_PARENTHESES || precedence < state.expressionsPrecedence.CallExpression || hasCallExpression(node2.callee) ? (state.write("("), this[node2.callee.type](node2.callee, state), state.write(")")) : this[node2.callee.type](node2.callee, state), formatSequence(state, node2.arguments);
  },
  CallExpression(node2, state) {
    let precedence = state.expressionsPrecedence[node2.callee.type];
    precedence === NEEDS_PARENTHESES || precedence < state.expressionsPrecedence.CallExpression ? (state.write("("), this[node2.callee.type](node2.callee, state), state.write(")")) : this[node2.callee.type](node2.callee, state), node2.optional && state.write("?."), formatSequence(state, node2.arguments);
  },
  ChainExpression(node2, state) {
    this[node2.expression.type](node2.expression, state);
  },
  MemberExpression(node2, state) {
    let precedence = state.expressionsPrecedence[node2.object.type];
    precedence === NEEDS_PARENTHESES || precedence < state.expressionsPrecedence.MemberExpression ? (state.write("("), this[node2.object.type](node2.object, state), state.write(")")) : this[node2.object.type](node2.object, state), node2.computed ? (node2.optional && state.write("?."), state.write("["), this[node2.property.type](node2.property, state), state.write("]")) : (node2.optional ? state.write("?.") : state.write("."), this[node2.property.type](node2.property, state));
  },
  ...
  Literal(node2, state) {
    node2.raw != null ? state.write(node2.raw, node2) : node2.regex != null ? this.RegExpLiteral(node2, state) : node2.bigint != null ? state.write(node2.bigint + "n", node2) : state.write(stringify(node2.value), node2);
  },
  RegExpLiteral(node2, state) {
    let { regex: regex2 } = node2;
    state.write(`/${regex2.pattern}/${regex2.flags}`, node2);
  }
}, EMPTY_OBJECT = {}, State = class {
  constructor(options) {
    let setup = options ?? EMPTY_OBJECT;
    this.output = "", setup.output != null ? (this.output = setup.output, this.write = this.writeToStream) : this.output = "", this.generator = setup.generator != null ? setup.generator : GENERATOR, this.expressionsPrecedence = setup.expressionsPrecedence != null ? setup.expressionsPrecedence : EXPRESSIONS_PRECEDENCE, this.indent = setup.indent != null ? setup.indent : "  ", this.lineEnd = setup.lineEnd != null ? setup.lineEnd : `
...
`).length - 1, this.mapping = {
  ...
  writeAndMap(code2, node2) {
    this.output += code2, this.map(code2, node2);
  }
  writeToStreamAndMap(code2, node2) {
    this.output.write(code2), this.map(code2, node2);
  }
  map(code2, node2) {
    if (node2 != null) {
      let { type } = node2;
      ...
      if (node2.loc != null) {
        let { mapping } = this;
        mapping.original = node2.loc.start, mapping.name = node2.name, this.sourceMap.addMapping(mapping);
      }
...
` ? (column = 0, line++) : column++;
    ...
    let { length } = code2, { lineEnd } = this;
    length > 0 && (this.lineEndSize > 0 && (lineEnd.length === 1 ? code2[length - 1] === lineEnd : code2.endsWith(lineEnd)) ? (this.line += this.lineEndSize, this.column = 0) : this.column += length);
  }
  toString() {
    return this.output;
  }
...
function toJs(tree, options) {
  let { SourceMapGenerator, filePath, handlers: handlers2 } = options || emptyOptions4, sourceMap = SourceMapGenerator ? new SourceMapGenerator({ file: filePath || "<unknown>.js" }) : void 0, value = generate(
    tree,
  ...
  ), map = sourceMap ? sourceMap.toJSON() : void 0;
  return { value, map };
}
var jsx = {
...
function jsxAttribute(node2, state) {
  this[node2.name.type](node2.name, state), node2.value !== null && node2.value !== void 0 && (state.write("="), node2.value.type === "Literal" ? state.write(
...
function jsxClosingElement(node2, state) {
  state.write("</"), this[node2.name.type](node2.name, state), state.write(">");
}
...
function jsxElement(node2, state) {
  let index2 = -1;
  if (this[node2.openingElement.type](node2.openingElement, state), node2.children)
    for (; ++index2 < node2.children.length; ) {
      let child = node2.children[index2];
  ...
  node2.closingElement && this[node2.closingElement.type](node2.closingElement, state);
}
function jsxEmptyExpression() {
}
function jsxExpressionContainer(node2, state) {
  state.write("{"), this[node2.expression.type](node2.expression, state), state.write("}");
}
function jsxFragment(node2, state) {
  let index2 = -1;
  if (this[node2.openingFragment.type](node2.openingFragment, state), node2.children)
    for (; ++index2 < node2.children.length; ) {
      let child = node2.children[index2];
      if (child.type === "JSXSpreadChild")
        throw new Error("JSX spread children are not supported");
...
function jsxMemberExpression(node2, state) {
  this[node2.object.type](node2.object, state), state.write("."), this[node2.property.type](node2.property, state);
}
function jsxNamespacedName(node2, state) {
  this[node2.namespace.type](node2.namespace, state), state.write(":"), this[node2.name.type](node2.name, state);
}
function jsxOpeningElement(node2, state) {
  let index2 = -1;
  if (state.write("<"), this[node2.name.type](node2.name, state), node2.attributes)
    for (; ++index2 < node2.attributes.length; )
      state.write(" "), this[node2.attributes[index2].type](node2.attributes[index2], state);
...
function jsxSpreadAttribute(node2, state) {
  state.write("{"), this.SpreadElement(node2, state), state.write("}");
}
...
function recmaStringify(options) {
  let self2 = (
    /** @type {Processor} */
    this
  ), { SourceMapGenerator } = options;
  self2.compiler = compiler2;
  function compiler2(tree, file) {
    let result = SourceMapGenerator ? toJs(tree, {
      SourceMapGenerator,
      filePath: file.path || "unknown.mdx",
      handlers: jsx
    }) : toJs(tree, { handlers: jsx });
    return file.map = result.map, result.value;
  }
}
  ...
  return function(tree) {
    visit2(tree, "raw", function(_, index2, parent) {
      if (parent && typeof index2 == "number")
        return parent.children.splice(index2, 1), index2;
    });
  };
}
var js = /\s+/g, html2 = /[\t\n\v\f\r ]+/g;
function collapseWhiteSpace(value, options) {
  ...
  let replace = options.preserveLineEndings ? replaceLineEnding : replaceSpace;
  return String(value).replace(
    options.style === "html" ? html2 : js,
...
function remarkMarkAndUnravel() {
  return function(tree) {
    visit2(tree, function(node2, index2, parent) {
      let offset2 = -1, all2 = !0, oneOrMore = !1;
      if (parent && typeof index2 == "number" && node2.type === "paragraph") {
        let children = node2.children;
        for (; ++offset2 < children.length; ) {
          let child = children[offset2];
          if (child.type === "mdxJsxTextElement" || child.type === "mdxTextExpression")
            oneOrMore = !0;
          else if (!(child.type === "text" && collapseWhiteSpace(child.value, { style: "html", trim: !0 }) === "")) {
            all2 = !1;
            break;
          }
        }
        if (all2 && oneOrMore) {
          ...
          for (; ++offset2 < children.length; ) {
            let child = children[offset2];
            child.type === "mdxJsxTextElement" && (child.type = "mdxJsxFlowElement"), child.type === "mdxTextExpression" && (child.type = "mdxFlowExpression"), child.type === "text" && /^[\t\r\n ]+$/.test(String(child.value)) || newChildren.push(child);
          }
          return parent.children.splice(index2, 1, ...newChildren), index2;
        }
      }
      if (node2.type === "mdxJsxFlowElement" || node2.type === "mdxJsxTextElement") {
        let data2 = node2.data || (node2.data = {});
        data2._mdxExplicitJsx = !0;
      }
    });
  };
}
...
function createProcessor(options) {
  let settings = options || {}, index2 = -1;
  for (; ++index2 < removedOptions.length; ) {
    let key = removedOptions[index2];
    key in settings && ("" + key, void 0);
  }
  settings.format === "detect" && void 0, (settings.jsxRuntime === "classic" || settings.pragma || settings.pragmaFrag || settings.pragmaImportSource) && !warned && (warned = !0, console.warn(
  ...
  settings.format !== "md" && pipeline.use(remarkMdx);
  let remarkRehypeOptions = settings.remarkRehypeOptions || {};
  return pipeline.use(remarkMarkAndUnravel).use(settings.remarkPlugins || []).use(remarkRehype, {
    ...remarkRehypeOptions,
    allowDangerousHtml: !0,
    passThrough: [...remarkRehypeOptions.passThrough || [], ...nodeTypes]
  }).use(settings.rehypePlugins || []), settings.format === "md" && pipeline.use(rehypeRemoveRaw), pipeline.use(rehypeRecma, settings).use(recmaDocument, settings).use(recmaJsxRewrite, settings), settings.jsx || pipeline.use(recmaJsxBuild, settings), pipeline.use(recmaStringify, settings).use(settings.recmaPlugins || []), pipeline;
}
...
async function evaluate(file, options) {
  let { compiletime, runtime } = resolveEvaluateOptions(options);
